...
const EXPECTED_REPOSITORY_URL = "https://github.com/quantumclaw/quantumclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "quantumclaw") {
    errors.push(`package.json name must be "quantumclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.quantumclaw !== "quantumclaw.mjs") {
    errors.push(
      `package.json bin.quantumclaw must be "quantumclaw.mjs"; found "${pkg.bin?.quantumclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`quantumclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `quantumclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
