...
IMAGE_NAME="quantumclaw-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  QUANTUMCLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  QUANTUMCLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export QUANTUMCLAW_ENTRY

home_dir=$(mktemp -d "/tmp/quantumclaw-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/quantumclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/quantumclaw.plugin.json" "$id"
...
mkdir -p "$HOME/.quantumclaw/extensions/demo-plugin"

cat > "$HOME/.quantumclaw/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.quantumclaw/extensions/demo-plugin/quantumclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/quantumclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@quantumclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "quantumclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/quantumclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$QUANTUMCLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/quantumclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@quantumclaw/demo-plugin-dir",
  "version": "0.0.1",
  "quantumclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/quantumclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$QUANTUMCLAW_ENTRY" plugins install "$dir_plugin"
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/quantumclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@quantumclaw/demo-plugin-file",
  "version": "0.0.1",
  "quantumclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/quantumclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$QUANTUMCLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "quantumclaw/fixture-marketplace"
    }
  }
}
JSON

node "$QUANTUMCLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$QUANTUMCLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$QUANTUMCLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".quantumclaw", "quantumclaw.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$QUANTUMCLAW_ENTRY" plugins update marketplace-shortcut --dry-run
node "$QUANTUMCLAW_ENTRY" plugins update marketplace-shortcut
node "$QUANTUMCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
