...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-quantumclaw-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://www.npmjs.com/package/quantumclaw/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/quantumclaw-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/quantumclaw-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-quantumclaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://www.npmjs.com/package/quantumclaw/install.ps1
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: quantumclaw@2026.3.13-beta.1
...
guest_run_quantumclaw() {
  ...
  stdout_name="quantumclaw-stdout-$RANDOM-$RANDOM.log"
  stderr_name="quantumclaw-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\quantumclaw.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
matches = re.findall(r"QuantumClaw [^\r\n]+ \([0-9a-f]{7,}\)", text)
...
resolve_latest_version() {
  ...
  npm view quantumclaw version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "quantumclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\QuantumClaw\\deps\" mkdir \"%LOCALAPPDATA%\\QuantumClaw\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/quantumclaw-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/quantumclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\quantumclaw.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\QuantumClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\quantumclaw.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_quantumclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="quantumclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="quantumclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="quantumclaw-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$quantumclaw = Join-Path \$env:APPDATA 'npm\quantumclaw.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$quantumclaw, \$log)
...
verify_gateway() {
  guest_run_quantumclaw "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_quantumclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_quantumclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_quantumclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_quantumclaw "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "quantumclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "quantumclaw-main-upgrade.tgz" || return $?
