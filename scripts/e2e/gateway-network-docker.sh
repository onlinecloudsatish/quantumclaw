...
IMAGE_NAME="quantumclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="quantumclaw-net-e2e-$$"
GW_NAME="quantumclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "QUANTUMCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "QUANTUMCLAW_SKIP_CHANNELS=1" \
  -e "QUANTUMCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "QUANTUMCLAW_SKIP_CRON=1" \
  -e "QUANTUMCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
