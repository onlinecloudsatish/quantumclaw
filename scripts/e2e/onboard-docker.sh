...
IMAGE_NAME="quantumclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/quantumclaw-bin:$PATH"
  mkdir -p /tmp/quantumclaw-bin
  cat > /tmp/quantumclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/quantumclaw-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$QUANTUMCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_quantumclaw_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/quantumclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/quantumclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$QUANTUMCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/quantumclaw-e2e-$1.XXXXXX"
  }

  set_isolated_quantumclaw_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export QUANTUMCLAW_HOME="$home_dir"
    export QUANTUMCLAW_STATE_DIR="$home_dir/.quantumclaw"
    export QUANTUMCLAW_CONFIG_PATH="$QUANTUMCLAW_STATE_DIR/quantumclaw.json"
    mkdir -p "$QUANTUMCLAW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    set_isolated_quantumclaw_env "$home_dir"
    node "$QUANTUMCLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$QUANTUMCLAW_STATE_DIR/workspace"
    config_path="$QUANTUMCLAW_CONFIG_PATH"
    sessions_dir="$QUANTUMCLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_quantumclaw_env "$home_dir"
	    ...
	    node "$QUANTUMCLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$QUANTUMCLAW_CONFIG_PATH" <<'"'"'JSON'"'"'
{
  "meta": {},
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$QUANTUMCLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$QUANTUMCLAW_ENTRY\" configure --section skills" send_skills_flow
