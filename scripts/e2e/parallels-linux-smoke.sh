...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/quantumclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/quantumclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/quantumclaw-install.sh
  guest_exec /usr/bin/env QUANTUMCLAW_NO_ONBOARD=1 bash /tmp/quantumclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec quantumclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec quantumclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" quantumclaw onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" quantumclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
