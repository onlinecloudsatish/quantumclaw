...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_QUANTUMCLAW_BIN="/opt/homebrew/bin/quantumclaw"
GUEST_QUANTUMCLAW_ENTRY="/opt/homebrew/lib/node_modules/quantumclaw/quantumclaw.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/quantumclaw-parallels-smoke.XXXXXX)"
...
spawn {*}$cmd
send -- "printf '__QUANTUMCLAW_READY__\\n'\r"
expect "__QUANTUMCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/quantumclaw-prl.sh <<'__QUANTUMCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__QUANTUMCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/quantumclaw-prl.sh; rc=\$?; rm -f /tmp/quantumclaw-prl.sh; printf '__QUANTUMCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__QUANTUMCLAW_RC__:(-?[0-9]+)} {
...
export QUANTUMCLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/quantumclaw-install.sh
bash /tmp/quantumclaw-install.sh${version_arg_q}
$GUEST_QUANTUMCLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_QUANTUMCLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/quantumclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/quantumclaw"; check_path "\$root/quantumclaw/extensions"; if [ -d "\$root/quantumclaw/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/quantumclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_QUANTUMCLAW_ENTRY" agent --agent main --message ping --json
}
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/quantumclaw-discord-token <<'__QUANTUMCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__QUANTUMCLAW_TOKEN__
cat >/tmp/quantumclaw-discord-guilds.json <<'__QUANTUMCLAW_GUILDS__'
$guilds_json
__QUANTUMCLAW_GUILDS__
token="\$(tr -d '\n' </tmp/quantumclaw-discord-token)"
guilds_json="\$(cat /tmp/quantumclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_QUANTUMCLAW_ENTRY channels status --probe --json
rm -f /tmp/quantumclaw-discord-token /tmp/quantumclaw-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "quantumclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "quantumclaw-main-upgrade.tgz"
