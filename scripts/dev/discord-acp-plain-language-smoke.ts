...
type DriverMode = "token" | "webhook" | "quantumclaw";
...
function resolveStateDir(): string {
  const override = process.env.QUANTUMCLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.QUANTUMCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".quantumclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver quantumclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) QuantumClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|quantumclaw> Driver transport mode (default: token)\n" +
    ...
    "  --quantumclaw-bin <path>        QuantumClaw CLI binary for driver=quantumclaw (default: quantumclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_AGENT\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  QUANTUMCLAW_DISCORD_SMOKE_QUANTUMCLAW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "quantumclaw"
        ? "quantumclaw"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.QUANTUMCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.QUANTUMCLAW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.QUANTUMCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.QUANTUMCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.QUANTUMCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const quantumclawBin =
    resolveArg("--quantumclaw-bin") || process.env.QUANTUMCLAW_DISCORD_SMOKE_QUANTUMCLAW_BIN || "quantumclaw";
...
async function quantumclawCliJson<T>(params: { quantumclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.quantumclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`quantumclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithQuantumClaw(params: {
  quantumclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await quantumclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    quantumclawBin: params.quantumclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "quantumclaw") {
    return await readMessagesWithQuantumClaw({
      quantumclawBin: params.args.quantumclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `quantumclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await quantumclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        quantumclawBin: args.quantumclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("quantumclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "quantumclaw"
            ? await readMessagesWithQuantumClaw({
                quantumclawBin: args.quantumclawBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from QuantumClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
