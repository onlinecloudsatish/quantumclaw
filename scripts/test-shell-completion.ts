 ...
 * `quantumclaw update`. Use it to verify the completion installation flow
...
\`quantumclaw update\`, \`quantumclaw doctor\`, and \`quantumclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# QuantumClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`quantumclaw completion --install\` later to enable.`));
    return;
  }
