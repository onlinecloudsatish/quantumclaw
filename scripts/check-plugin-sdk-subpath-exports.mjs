...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("quantumclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("quantumclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced quantumclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced quantumclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
