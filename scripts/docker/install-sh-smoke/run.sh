...
INSTALL_URL="${QUANTUMCLAW_INSTALL_URL:-https://quantumclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${QUANTUMCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${QUANTUMCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="quantumclaw"
PACKAGE_NAME="${QUANTUMCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (QUANTUMCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${QUANTUMCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${QUANTUMCLAW_INSTALL_LATEST_OUT:-}"
fi
