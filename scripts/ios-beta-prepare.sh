...
- regenerates apps/ios/QuantumClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${QUANTUMCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid QUANTUMCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid QUANTUMCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid QUANTUMCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid QUANTUMCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing QUANTUMCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(QUANTUMCLAW_URL_SLASH)$(QUANTUMCLAW_URL_SLASH)#g'
)"
...
QUANTUMCLAW_CODE_SIGN_STYLE = Automatic
QUANTUMCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
QUANTUMCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
QUANTUMCLAW_APP_BUNDLE_ID = ai.quantumclaw.client
QUANTUMCLAW_SHARE_BUNDLE_ID = ai.quantumclaw.client.share
QUANTUMCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.quantumclaw.client.activitywidget
QUANTUMCLAW_WATCH_APP_BUNDLE_ID = ai.quantumclaw.client.watchkitapp
QUANTUMCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.quantumclaw.client.watchkitapp.extension
QUANTUMCLAW_APP_PROFILE =
QUANTUMCLAW_SHARE_PROFILE =
QUANTUMCLAW_PUSH_TRANSPORT = relay
QUANTUMCLAW_PUSH_DISTRIBUTION = official
QUANTUMCLAW_URL_SLASH = /
QUANTUMCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
QUANTUMCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
