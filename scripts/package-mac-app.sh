...
# Build and bundle QuantumClaw into a minimal .app we can open.
# Outputs to dist/QuantumClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/QuantumClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="QuantumClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.quantumclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/quantumclaw/quantumclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/QuantumClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :QuantumClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :QuantumClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/QuantumClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/QuantumClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/QuantumClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/QuantumClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/QuantumClaw/Resources/QuantumClaw.icns" "$APP_ROOT/Contents/Resources/QuantumClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/QuantumClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying QuantumClawKit resources"
QUANTUMCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/QuantumClawKit_QuantumClawKit.bundle"
if [ -d "$QUANTUMCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/QuantumClawKit_QuantumClawKit.bundle"
  cp -R "$QUANTUMCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/QuantumClawKit_QuantumClawKit.bundle"
else
  echo "WARN: QuantumClawKit resource bundle not found at $QUANTUMCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running QuantumClaw"
killall -q QuantumClaw 2>/dev/null || true
