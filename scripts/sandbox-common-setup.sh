...
BASE_IMAGE="${BASE_IMAGE:-quantumclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-quantumclaw-sandbox-common:bookworm-slim}"
...
QUANTUMCLAW_DOCKER_BUILD_USE_BUILDX="${QUANTUMCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
QUANTUMCLAW_DOCKER_BUILD_CACHE_FROM="${QUANTUMCLAW_DOCKER_BUILD_CACHE_FROM:-}"
QUANTUMCLAW_DOCKER_BUILD_CACHE_TO="${QUANTUMCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${QUANTUMCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${QUANTUMCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${QUANTUMCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${QUANTUMCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${QUANTUMCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=quantumclaw.sandbox=1)
NOTE
