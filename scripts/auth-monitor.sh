...
#
# Suggested cron: */30 * * * * /home/admin/quantumclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send QuantumClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., quantumclaw-alerts)
...
STATE_FILE="$HOME/.quantumclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via QuantumClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use quantumclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via QuantumClaw to $NOTIFY_PHONE..."
            quantumclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: QuantumClaw Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! QuantumClaw is down. Run: ssh l36 '~/quantumclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
