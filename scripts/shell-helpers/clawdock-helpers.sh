#!/usr/bin/env bash
# ClawDock - Docker helpers for QuantumClaw
# Inspired by Simon Willison's "Running QuantumClaw in Docker"
# https://til.simonwillison.net/llms/quantumclaw-docker
#
# Installation:
...
# Common paths to check for QuantumClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/quantumclaw"
  "${HOME}/workspace/quantumclaw"
  "${HOME}/projects/quantumclaw"
  "${HOME}/dev/quantumclaw"
  "${HOME}/code/quantumclaw"
  "${HOME}/src/quantumclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found QuantumClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/quantumclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ QuantumClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/quantumclaw/quantumclaw.git ~/quantumclaw"
    echo "  cd ~/quantumclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^QUANTUMCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d quantumclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart quantumclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f quantumclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.quantumclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec quantumclaw-gateway \
    bash -c 'echo "alias quantumclaw=\"./quantumclaw.mjs\"" > /tmp/.bashrc_quantumclaw && bash --rcfile /tmp/.bashrc_quantumclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm quantumclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build quantumclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "QUANTUMCLAW_GATEWAY_TOKEN=$token" quantumclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" quantumclaw-gateway \
    bash -c './quantumclaw.mjs config set gateway.remote.token "$TOKEN" && ./quantumclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec quantumclaw-gateway \
    bash -c "./quantumclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart quantumclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm quantumclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec quantumclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'quantumclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for QuantumClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (quantumclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to quantumclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.quantumclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'quantumclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'quantumclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.www.npmjs.com/package/quantumclaw${_CLR_RESET}"
  echo ""
}
