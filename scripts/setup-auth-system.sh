#!/bin/bash
# Setup QuantumClaw Auth Management System
...
echo "=== QuantumClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'quantumclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  QuantumClaw message: Send warning via QuantumClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/quantumclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/quantumclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/quantumclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now quantumclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status quantumclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
