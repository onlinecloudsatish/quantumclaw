...
function stripWorkspaceQuantumClawDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.quantumclaw !== "workspace:*") {
    return false;
  }
  delete devDeps.quantumclaw;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceQuantumClawDevDependency(pkg);
