...
export function requireQuantumClawAgentDir(): string {
  const agentDir = process.env.QUANTUMCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("QUANTUMCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
