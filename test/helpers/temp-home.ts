...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked QUANTUMCLAW_HOME.
  delete process.env.QUANTUMCLAW_HOME;
  process.env.QUANTUMCLAW_STATE_DIR = path.join(base, ".quantumclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "quantumclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".quantumclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
