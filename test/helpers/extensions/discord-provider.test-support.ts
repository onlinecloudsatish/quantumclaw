...
import type { QuantumClawConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: QuantumClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): QuantumClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("quantumclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/acp-runtime")>(
    "quantumclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("quantumclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/command-auth")>(
...
vi.mock("quantumclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/reply-runtime")>(
    "quantumclaw/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("quantumclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/runtime-env")>(
    "quantumclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("quantumclaw/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/infra-runtime")>(
