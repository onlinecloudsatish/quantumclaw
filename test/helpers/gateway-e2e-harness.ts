...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `quantumclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".quantumclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          QUANTUMCLAW_SKIP_PROVIDERS: "1",
          QUANTUMCLAW_SKIP_GMAIL_WATCHER: "1",
          QUANTUMCLAW_SKIP_CRON: "1",
          QUANTUMCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          QUANTUMCLAW_SKIP_CANVAS_HOST: "1",
          QUANTUMCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
