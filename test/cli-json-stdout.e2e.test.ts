  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.QUANTUMCLAW_HOME;
        delete env.QUANTUMCLAW_STATE_DIR;
        delete env.QUANTUMCLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "quantumclaw.mjs");
