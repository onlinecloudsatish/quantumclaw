...
const QUANTUMCLAW_OPENSHELL_E2E = process.env.QUANTUMCLAW_E2E_OPENSHELL === "1";
const QUANTUMCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const QUANTUMCLAW_OPENSHELL_COMMAND =
  process.env.QUANTUMCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "quantumclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && QUANTUMCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: QUANTUMCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(QUANTUMCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-openshell-e2e-"));
      ...
      const gatewayName = `quantumclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `quantumclaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: QUANTUMCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("quantumclaw-openshell-e2e");
