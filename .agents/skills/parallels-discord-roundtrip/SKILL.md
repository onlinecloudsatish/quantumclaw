...
```bash
export QUANTUMCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.quantumclaw/quantumclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env QUANTUMCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `quantumclaw` wrapper for guest `message send/read`; `node quantumclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/quantumclaw-parallels-smoke.*`
...
- host inbound nonce appears in `quantumclaw message read` output
