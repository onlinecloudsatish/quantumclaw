# QuantumClaw ACP Bridge

This document describes how the QuantumClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`quantumclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
QuantumClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing QuantumClaw Gateway
...
- ACP client spawns `quantumclaw acp` and speaks ACP messages over stdio.
...
`quantumclaw acp` resolves the Gateway URL and auth from CLI flags or config:
