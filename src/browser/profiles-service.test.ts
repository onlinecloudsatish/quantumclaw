...
vi.mock("./chrome.js", () => ({
  resolveQuantumClawUserDataDir: vi.fn(() => "/tmp/quantumclaw-test/quantumclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "quantumclaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveQuantumClawUserDataDir).mockReturnValue(userDataDir);
