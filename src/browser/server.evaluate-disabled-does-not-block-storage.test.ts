  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          quantumclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.QUANTUMCLAW_GATEWAY_PORT;
    process.env.QUANTUMCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.QUANTUMCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.QUANTUMCLAW_GATEWAY_PASSWORD;
