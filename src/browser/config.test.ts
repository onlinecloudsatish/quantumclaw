...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("quantumclaw");
    expect(profile?.driver).toBe("quantumclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const quantumclaw = resolveProfile(resolved, "quantumclaw");
    expect(quantumclaw?.driver).toBe("quantumclaw");
    expect(quantumclaw?.cdpPort).toBe(18800);
    expect(quantumclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from QUANTUMCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ QUANTUMCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const quantumclaw = resolveProfile(resolved, "quantumclaw");
      expect(quantumclaw?.cdpPort).toBe(19012);
      expect(quantumclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ QUANTUMCLAW_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const quantumclaw = resolveProfile(resolved, "quantumclaw");
      expect(quantumclaw?.cdpPort).toBe(19022);
      expect(quantumclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(quantumclaw?.cdpPort).toBe(19000);
    expect(quantumclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "quantumclaw");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "quantumclaw")!;
  ...
  describe("default profile preference", () => {
    it("defaults to quantumclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("quantumclaw");
    });

    it("keeps quantumclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("quantumclaw");
    ...
    it("keeps quantumclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("quantumclaw");
    ...
    it("keeps quantumclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
