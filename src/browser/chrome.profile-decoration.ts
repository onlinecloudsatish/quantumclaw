...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".quantumclaw-profile-decorated");
}
 ...
 */
export function decorateQuantumClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_QUANTUMCLAW_BROWSER_COLOR).toUpperCase();
