import type { BrowserConfig, BrowserProfileConfig, QuantumClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_QUANTUMCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_QUANTUMCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "quantumclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: QuantumClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_QUANTUMCLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "quantumclaw";
