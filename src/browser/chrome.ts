...
export function resolveQuantumClawUserDataDir(profileName = DEFAULT_QUANTUMCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchQuantumClawChrome(
  ...
  const userDataDir = resolveQuantumClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_QUANTUMCLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateQuantumClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 quantumclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`quantumclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`quantumclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 quantumclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopQuantumClawChrome(
