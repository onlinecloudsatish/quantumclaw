...
function localQuantumClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "quantumclaw",
...
function createLocalQuantumClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localQuantumClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveQuantumClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-reset-"));
    const profileDir = path.join(tempRoot, "quantumclaw");
    ...
    const ops = createLocalQuantumClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveQuantumClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "quantumclaw");
    fs.mkdirSync(profileDir, { recursive: true });
