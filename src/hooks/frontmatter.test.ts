...
`;
    ...
    expect(parsed.quantumclaw.emoji).toBe("💾");
    expect(parsed.quantumclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.quantumclaw.emoji).toBe("📝");
    expect(parsed.quantumclaw.events).toEqual(["command"]);
    expect(parsed.quantumclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.quantumclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"quantumclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"quantumclaw": {"events": ["test"]}}');
  });
...
describe("resolveQuantumClawMetadata", () => {
  it("extracts quantumclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveQuantumClawMetadata(frontmatter);
  ...
  it("returns undefined when quantumclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        quantumclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with QuantumClaw" },
            { id: "npm", kind: "npm", package: "@quantumclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@quantumclaw/hook");
  });
...
`;
    ...
    const quantumclaw = resolveQuantumClawMetadata(frontmatter);
    expect(quantumclaw).toBeDefined();
    expect(quantumclaw?.emoji).toBe("💾");
    expect(quantumclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(quantumclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(quantumclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const quantumclaw = resolveQuantumClawMetadata(frontmatter);
    expect(quantumclaw?.emoji).toBe("disk");
    expect(quantumclaw?.events).toEqual(["command:new"]);
  });
});
