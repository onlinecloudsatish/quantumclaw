...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "quantumclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.QUANTUMCLAW_BUNDLED_HOOKS_DIR;
    process.env.QUANTUMCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.QUANTUMCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.QUANTUMCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): QuantumClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("quantumclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as QuantumClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".quantumclaw", "extensions", "claude-bundle");
