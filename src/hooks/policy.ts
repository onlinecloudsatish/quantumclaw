import type { QuantumClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "quantumclaw-bundled": {
    ...
    canOverride: ["quantumclaw-bundled"],
    canBeOverriddenBy: ["quantumclaw-managed", "quantumclaw-plugin"],
  },
  "quantumclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["quantumclaw-bundled", "quantumclaw-plugin"],
    canBeOverriddenBy: ["quantumclaw-managed"],
  },
  "quantumclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["quantumclaw-bundled", "quantumclaw-managed", "quantumclaw-plugin"],
    canBeOverriddenBy: ["quantumclaw-managed"],
  },
  "quantumclaw-workspace": {
    ...
    canOverride: ["quantumclaw-workspace"],
    canBeOverriddenBy: ["quantumclaw-workspace"],
  },
};
...
export function resolveHookConfig(
  config: QuantumClawConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "quantumclaw-plugin") {
    return { enabled: true };
  }
