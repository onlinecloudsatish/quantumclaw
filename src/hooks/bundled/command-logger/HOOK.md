...
2. **Appends to log file** - Writes a JSON line to `~/.quantumclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.quantumclaw/logs/commands.log ~/.quantumclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/quantumclaw`:
   ```
   /home/username/.quantumclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
