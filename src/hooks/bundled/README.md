...
This directory contains hooks that ship with QuantumClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.quantumclaw/quantumclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'quantumclaw.*gateway' && pnpm quantumclaw gateway`
3. Enable the hook: `quantumclaw hooks enable my-hook`
