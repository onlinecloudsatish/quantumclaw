...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "quantumclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "quantumclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "quantumclaw-bundled");
      const plugin = makeHookEntry("shared", "quantumclaw-plugin");
      const managed = makeHookEntry("shared", "quantumclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("quantumclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "quantumclaw-managed");
      const workspace = makeHookEntry("shared", "quantumclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "quantumclaw-workspace");
      const second = makeHookEntry("shared", "quantumclaw-workspace");
      second.hook.handlerPath = "/tmp/quantumclaw-workspace/shared/handler-2.js";
