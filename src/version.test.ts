...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "quantumclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "quantumclaw", version: "2.3.4" });
  ...
  it("ignores non-quantumclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers QUANTUMCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        QUANTUMCLAW_VERSION: "9.9.9",
        QUANTUMCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        QUANTUMCLAW_VERSION: "   ",
        QUANTUMCLAW_SERVICE_VERSION: "  2.0.0  ",
