...
describe("usage-format", () => {
  const originalAgentDir = process.env.QUANTUMCLAW_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.QUANTUMCLAW_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over quantumclaw config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to quantumclaw config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {
