...
describe("canvas host state dir defaults", () => {
  ...
  it("uses QUANTUMCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("quantumclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("QuantumClaw Canvas");
      } finally {
        await handler.close();
      }
    });
