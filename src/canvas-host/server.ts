...
<title>QuantumClaw Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.quantumclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.quantumclawCanvasA2UIAction &&
        typeof window.quantumclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.quantumclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("quantumclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android QuantumClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.quantumclawSendUserAction === "function"
        ? window.quantumclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.QUANTUMCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.QUANTUMCLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>QuantumClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
