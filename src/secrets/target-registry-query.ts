...
const QUANTUMCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of QUANTUMCLAW_COMPILED_SECRET_TARGETS) {
...
const QUANTUMCLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: QUANTUMCLAW_COMPILED_SECRET_TARGETS,
    entriesById: QUANTUMCLAW_TARGETS_BY_ID,
  });
