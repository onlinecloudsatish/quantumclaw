...
import { clearConfigCache, type QuantumClawConfig } from "../config/config.js";
...
function createOpenAiFileModelsConfig(): NonNullable<QuantumClawConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: QuantumClawConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-secrets-runtime-"));
    ...
    const prevStateDir = process.env.QUANTUMCLAW_STATE_DIR;
    ...
    try {
      ...
      process.env.QUANTUMCLAW_STATE_DIR = stateDir;
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.QUANTUMCLAW_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
