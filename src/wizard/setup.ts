...
export async function runSetupWizard(
  ...
  await prompter.intro("QuantumClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: QuantumClawConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("quantumclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (compatibilityNotices.length > 0) {
    await prompter.note(
      [
        ...
        `Inspect: ${formatCliCommand("quantumclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("quantumclaw configure")}.`;
  ...
  let localGatewayToken = process.env.QUANTUMCLAW_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.QUANTUMCLAW_GATEWAY_PASSWORD ?? process.env.CLAWDBOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: QuantumClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
