...
let lastCreateQuantumClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/quantumclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateQuantumClawToolsContext?.agentTo,
          agentThreadId: lastCreateQuantumClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createQuantumClawTools: (ctx: Record<string, unknown>) => {
      lastCreateQuantumClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateQuantumClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateQuantumClawToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateQuantumClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-quantumclaw-message-to": "channel:24514",
        "x-quantumclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
