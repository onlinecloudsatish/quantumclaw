...
import type { ConfigValidationIssue, QuantumClawConfig } from "../../config/types.quantumclaw.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: QuantumClawConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadQuantumClawPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    runtimeOptions: {
  ...
  // our cache. However, both loadConfig() and loadQuantumClawPlugins() already cache
