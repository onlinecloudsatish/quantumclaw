...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/quantumclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (QuantumClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.QUANTUMCLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "quantumclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.QUANTUMCLAW_TAILNET_DNS?.trim();
