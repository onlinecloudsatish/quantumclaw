...
const loadQuantumClawPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadQuantumClawPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadQuantumClawPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadQuantumClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadQuantumClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.quantumclaw.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
