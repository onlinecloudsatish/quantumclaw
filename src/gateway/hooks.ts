...
export function resolveHooksConfig(cfg: QuantumClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: QuantumClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-quantumclaw-token"] === "string"
      ? req.headers["x-quantumclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-quantumclaw-idempotency-key"]) ||
