...
import { resolveQuantumClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureQuantumClawModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, QuantumClawConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.QUANTUMCLAW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.QUANTUMCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.QUANTUMCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.QUANTUMCLAW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.QUANTUMCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.QUANTUMCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.QUANTUMCLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: QuantumClawConfig;
  agentDir: string;
}): QuantumClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<QuantumClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.QUANTUMCLAW_CONFIG_PATH,
    token: process.env.QUANTUMCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.QUANTUMCLAW_SKIP_CHANNELS,
    skipGmail: process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.QUANTUMCLAW_SKIP_CRON,
    skipCanvas: process.env.QUANTUMCLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.QUANTUMCLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.QUANTUMCLAW_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveQuantumClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-live-state-"));
  process.env.QUANTUMCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.QUANTUMCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.quantumclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveQuantumClawAgentDir();
  const sanitizedCfg: QuantumClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-live-"));
  const tempConfigPath = path.join(tempDir, "quantumclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.QUANTUMCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              message: strictReply
                ? "QuantumClaw live tool probe (local, safe): " +
    ...
    process.env.QUANTUMCLAW_CONFIG_PATH = previous.configPath;
    process.env.QUANTUMCLAW_GATEWAY_TOKEN = previous.token;
    process.env.QUANTUMCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.QUANTUMCLAW_SKIP_CRON = previous.skipCron;
    process.env.QUANTUMCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.QUANTUMCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.QUANTUMCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureQuantumClawModelsJson(cfg);
      ...
      const rawModels = process.env.QUANTUMCLAW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via QUANTUMCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.quantumclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
