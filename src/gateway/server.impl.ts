...
import { ensureQuantumClawCliOnPath } from "../infra/path-env.js";
import {
...
import {
  attachQuantumClawTranscriptMeta,
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("quantumclaw doctor")}" to repair, then retry.`
    : "";
...
async function prepareGatewayStartupConfig(params: {
  ...
  ) => Promise<{ config: QuantumClawConfig }>;
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.QUANTUMCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.QUANTUMCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "QUANTUMCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "QUANTUMCLAW_RAW_STREAM_PATH",
  ...
  let cfgAtStart: QuantumClawConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `quantumclaw config set gateway.auth.mode token` and `quantumclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "quantumclaw plugins install @quantumclaw/matrix",
      repoInstallCommand: "quantumclaw plugins install ./extensions/matrix",
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachQuantumClawTranscriptMeta(update.message, {
