...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "QUANTUMCLAW_BUNDLED_PLUGINS_DIR",
  "QUANTUMCLAW_SKIP_CHANNELS",
  "QUANTUMCLAW_SKIP_PROVIDERS",
  "QUANTUMCLAW_SKIP_CRON",
  "QUANTUMCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionStorePath(storePath: string): Promise<void> {
  ...
  if (process.env.QUANTUMCLAW_CONFIG_PATH) {
    configPaths.add(process.env.QUANTUMCLAW_CONFIG_PATH);
  }
  if (process.env.QUANTUMCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.QUANTUMCLAW_STATE_DIR, "quantumclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.QUANTUMCLAW_SKIP_PROVIDERS = "1";
  process.env.QUANTUMCLAW_SKIP_CRON = "1";
  process.env.QUANTUMCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.QUANTUMCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "quantumclaw-test-no-bundled-extensions")
    : "quantumclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".quantumclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".quantumclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__quantumclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__quantumclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["QUANTUMCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.QUANTUMCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.QUANTUMCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.QUANTUMCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
