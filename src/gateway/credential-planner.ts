...
type GatewaySecretDefaults = NonNullable<QuantumClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${QUANTUMCLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["QUANTUMCLAW_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["QUANTUMCLAW_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
