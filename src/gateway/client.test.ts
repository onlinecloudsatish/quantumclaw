...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("quantumclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with QUANTUMCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with QUANTUMCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.QUANTUMCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://quantumclaw-gateway.ai:18789",
      onConnectError,
    });
