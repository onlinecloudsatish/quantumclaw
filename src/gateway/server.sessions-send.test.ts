...
const { createQuantumClawTools } = await import("../agents/quantumclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createQuantumClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createQuantumClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["QUANTUMCLAW_GATEWAY_PORT", "QUANTUMCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.QUANTUMCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.QUANTUMCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("QUANTUMCLAW_CONFIG_PATH missing in gateway test environment");
      }
