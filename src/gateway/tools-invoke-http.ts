...
import { createQuantumClawTools } from "../agents/quantumclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-quantumclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-quantumclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-quantumclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-quantumclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createQuantumClawTools({
