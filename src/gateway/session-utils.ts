...
import { type QuantumClawConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: QuantumClawConfig): string[] {
...
export function listAgentsForGateway(cfg: QuantumClawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: QuantumClawConfig): string {
...
function resolveSessionStoreAgentId(cfg: QuantumClawConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: QuantumClawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: QuantumClawConfig): GatewaySessionsDefaults {
