...
function cfg(input: Partial<QuantumClawConfig>): QuantumClawConfig {
  return input as QuantumClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: QuantumClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  QUANTUMCLAW_GATEWAY_TOKEN: "env-token",
  QUANTUMCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when QUANTUMCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when QUANTUMCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
