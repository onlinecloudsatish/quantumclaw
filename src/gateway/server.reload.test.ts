...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.QUANTUMCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.QUANTUMCLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.QUANTUMCLAW_SKIP_CHANNELS = "0";
    delete process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER;
    delete process.env.QUANTUMCLAW_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.QUANTUMCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.QUANTUMCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.QUANTUMCLAW_SKIP_PROVIDERS;
    } else {
      process.env.QUANTUMCLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.QUANTUMCLAW_CONFIG_PATH;
    if (!configPath) {
      throw new Error("QUANTUMCLAW_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.QUANTUMCLAW_STATE_DIR;
    if (!stateDir) {
      throw new Error("QUANTUMCLAW_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_QUANTUMCLAW_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_QUANTUMCLAW_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_QUANTUMCLAW_AUTH_REF" is missing or empty.',
      );
    } finally {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.QUANTUMCLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.QUANTUMCLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
      envSnapshot.restore();
      ws.close();
      await server.close();
