...
const connectNodeClient = async (params: {
  ...
  const token = process.env.QUANTUMCLAW_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("QUANTUMCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".quantumclaw", "restart-sentinel.json");
