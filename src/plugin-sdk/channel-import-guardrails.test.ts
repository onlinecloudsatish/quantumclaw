...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/discord["']/, /plugin-sdk-internal\/discord/],
  },
  {
    path: "extensions/slack/src/shared.ts",
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/slack["']/, /plugin-sdk-internal\/slack/],
  },
  {
    path: "extensions/telegram/src/shared.ts",
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/telegram["']/, /plugin-sdk-internal\/telegram/],
  },
  {
    path: "extensions/imessage/src/shared.ts",
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/imessage["']/, /plugin-sdk-internal\/imessage/],
  },
  {
    path: "extensions/whatsapp/src/shared.ts",
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/whatsapp["']/, /plugin-sdk-internal\/whatsapp/],
  },
  {
    path: "extensions/signal/src/shared.ts",
    forbiddenPatterns: [/["']quantumclaw\/plugin-sdk\/signal["']/, /plugin-sdk-internal\/signal/],
  },
];
...
describe("channel import guardrails", () => {
  ...
  it("keeps bundled extension source files off root and compat plugin-sdk imports", () => {
    for (const file of collectExtensionSourceFiles()) {
      ...
      expect(text, `${file} should not import quantumclaw/plugin-sdk root`).not.toMatch(
        /["']quantumclaw\/plugin-sdk["']/,
      );
      expect(text, `${file} should not import quantumclaw/plugin-sdk/compat`).not.toMatch(
        /["']quantumclaw\/plugin-sdk\/compat["']/,
      );
    }
  });
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']quantumclaw/plugin-sdk/${extensionId}["']`, "u"));
      }
    }
  });
});
