...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-install-sh-"));
  const cliPath = path.join(root, "quantumclaw");
...
QUANTUMCLAW_BIN="$FAKE_QUANTUMCLAW_BIN"
resolve_quantumclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_QUANTUMCLAW_BIN: cliPath,
        QUANTUMCLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("QuantumClaw 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("QuantumClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("QuantumClaw dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-install-stdin-"));
