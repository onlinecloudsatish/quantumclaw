...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        QUANTUMCLAW_BASE_ENV: "base",
        QUANTUMCLAW_TO_REMOVE: undefined,
      },
      env: {
        QUANTUMCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.QUANTUMCLAW_BASE_ENV).toBe("base");
    expect(resolved.QUANTUMCLAW_TEST_ENV).toBe("ok");
    expect(resolved.QUANTUMCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.QUANTUMCLAW_CLI).toBe(QUANTUMCLAW_CLI_ENV_VALUE);
  });
