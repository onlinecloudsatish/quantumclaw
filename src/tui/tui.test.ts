...
describe("resolveInitialTuiAgentId", () => {
  const cfg: QuantumClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/quantumclaw" },
        { id: "ops", workspace: "/tmp/quantumclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/quantumclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run quantumclaw devices list");
    expect(state.pairingHint).toContain("quantumclaw devices list");
  });
