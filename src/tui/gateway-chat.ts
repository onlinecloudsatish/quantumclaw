...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set QUANTUMCLAW_GATEWAY_TOKEN/QUANTUMCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "quantumclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.QUANTUMCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.QUANTUMCLAW_GATEWAY_PASSWORD);
