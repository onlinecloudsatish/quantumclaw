...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: QuantumClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: QuantumClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/quantumclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<QuantumClawConfig["bindings"]>[number];
