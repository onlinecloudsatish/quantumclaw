...
function listAgents(cfg: QuantumClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: QuantumClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<QuantumClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: QuantumClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: QuantumClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: QuantumClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<QuantumClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  QuantumClawConfig,
  {
    bindingsRef: QuantumClawConfig["bindings"];
    agentsRef: QuantumClawConfig["agents"];
    sessionRef: QuantumClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: QuantumClawConfig): Map<string, ResolvedAgentRoute> {
