...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: QuantumClawConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: QuantumClawConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: QuantumClawConfig, policy: DmPolicy, accountId?: string) => QuantumClawConfig;
