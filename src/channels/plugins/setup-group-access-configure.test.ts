...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: QuantumClawConfig, policy: ChannelAccessPolicy) => QuantumClawConfig;
  resolveAllowlist: (params: { cfg: QuantumClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: QuantumClawConfig; resolved: TResolved }) => QuantumClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: QuantumClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: QuantumClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: QuantumClawConfig, policy: ChannelAccessPolicy): QuantumClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: QuantumClawConfig, policy: ChannelAccessPolicy): QuantumClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: QuantumClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: QuantumClawConfig; resolved: string[] }) => {
      calls.push("apply");
