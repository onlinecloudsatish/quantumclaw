...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: QuantumClawConfig) => Array<string | number> | undefined;
}): QuantumClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: QuantumClawConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: QuantumClawConfig) => Array<string | number> | undefined;
}): (cfg: QuantumClawConfig, dmPolicy: DmPolicy) => QuantumClawConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: QuantumClawConfig, allowFrom: string[]) => QuantumClawConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: QuantumClawConfig, groupPolicy: "open" | "allowlist" | "disabled") => QuantumClawConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: QuantumClawConfig) => QuantumClawConfig | Promise<QuantumClawConfig>;
  applySet?: (
    cfg: QuantumClawConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => QuantumClawConfig | Promise<QuantumClawConfig>;
}): Promise<{
  cfg: QuantumClawConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in QuantumClaw config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "QUANTUMCLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "QUANTUMCLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends QuantumClawConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: QuantumClawConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: QuantumClawConfig) => Array<string | number>;
