...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: QuantumClawConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as QuantumClawConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as QuantumClawConfig);
