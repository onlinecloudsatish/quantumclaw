...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@quantumclaw/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-catalog-"));
        ...
        entries: [
          {
            ...
            quantumclaw: {
              ...
              install: {
                npmSpec: "@quantumclaw/demo-channel",
              },
            },
          },
        ],
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@quantumclaw/env-demo-channel",
            quantumclaw: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@quantumclaw/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        QUANTUMCLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@quantumclaw/default-env-demo",
            quantumclaw: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@quantumclaw/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "quantumclaw" }),
      "utf8",
    );
    fs.writeFileSync(
      path.join(bundledDir, "package.json"),
      JSON.stringify({
        name: "@quantumclaw/whatsapp",
        quantumclaw: {
          extensions: ["./index.js"],
          channel: {
            id: "whatsapp",
    ...
    fs.writeFileSync(
      path.join(bundledDir, "quantumclaw.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        QUANTUMCLAW_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@quantumclaw/whatsapp");
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: QuantumClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
