...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-meta-");
    ...
    writeJson(path.join(pluginDir, "quantumclaw.plugin.json"), {
      id: "acpx",
      configSchema: { type: "object" },
      skills: ["./skills"],
    });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@quantumclaw/acpx-plugin",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "quantumclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "acpx", "quantumclaw.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
    ...
    ) as { quantumclaw?: { extensions?: string[] } };
    expect(packageJson.quantumclaw?.extensions).toEqual(["./index.js"]);
  });

  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-node-modules-");
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "tlon", "quantumclaw.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-hoisted-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-missing-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "quantumclaw.plugin.json"), {
      id: "tlon",
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-retry-");
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "quantumclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
      skills: ["./bundled-skills/@scope/skill"],
    });
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "package.json"), {
      name: "@quantumclaw/removed-plugin",
    });
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("quantumclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "quantumclaw.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@quantumclaw/google-gemini-cli-auth",
    });
