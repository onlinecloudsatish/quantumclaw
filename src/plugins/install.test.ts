...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      quantumclaw: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(bundleTempRoot, "quantumclaw-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@quantumclaw/cognee-quantumclaw",
      version: "0.0.1",
      quantumclaw: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "quantumclaw.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@quantumclaw/voice-call" });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@quantumclaw/zipper" });
  });

  it("installs flat-root plugin archives from ClawHub-style downloads", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
        name: "@quantumclaw/rootless",
        version: "0.0.1",
        quantumclaw: { extensions: ["./dist/index.js"] },
      },
  ...
  it("rejects packages without quantumclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@quantumclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("quantumclaw.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_QUANTUMCLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when quantumclaw.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@quantumclaw/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing quantumclaw.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_QUANTUMCLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without quantumclaw.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.quantumclaw).toBeUndefined();
  ...
  it("rejects plugins whose minHostVersion is newer than the current host", async () => {
    vi.stubEnv("QUANTUMCLAW_VERSION", "2026.3.21");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      quantumclaw?: { install?: Record<string, unknown> };
    };
    manifest.quantumclaw = {
      ...manifest.quantumclaw,
      install: {
        ...manifest.quantumclaw?.install,
    ...
    expect(result.error).toContain("requires QuantumClaw >=2026.3.22, but this host is 2026.3.21");
  ...
  it("rejects plugins with invalid minHostVersion metadata", async () => {
    ...
    expect(result.error).toContain("invalid package.json quantumclaw.install.minHostVersion");
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });

  it("reports unknown host versions distinctly for minHostVersion-gated plugins", async () => {
    vi.stubEnv("QUANTUMCLAW_VERSION", "unknown");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      quantumclaw?: { install?: Record<string, unknown> };
  ...
  it("uses quantumclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@quantumclaw/cognee-quantumclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("keeps scoped install ids aligned across manifest and package-name cases", async () => {
    const scenarios = [
      ...
      },
      {
        ...
        expectedPluginId: "@quantumclaw/test-plugin",
        install: (pluginDir: string, extensionsDir: string) =>
          installPluginFromDir({
            dirPath: pluginDir,
            extensionsDir,
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@quantumclaw/voice-call@0.0.1",
              name: "@quantumclaw/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@quantumclaw/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@quantumclaw/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@quantumclaw/voice-call@0.0.1",
    });
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@quantumclaw/voice-call@0.0.2-beta.1",
      name: "@quantumclaw/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@quantumclaw/voice-call@beta"');
      }
    }
...

    {
      ...
      const result = await installPluginFromNpmSpec({
        spec: "@quantumclaw/voice-call@beta",
        extensionsDir,
        logger: { info: () => {}, warn: () => {} },
      });
      expect(result.ok).toBe(true);
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@quantumclaw/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls,
        expectedSpec: "@quantumclaw/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
