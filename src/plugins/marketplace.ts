...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-marketplace-"));
...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-marketplace-download-"));
