...
function createSinglePluginWithEmptySlotsConfig(): QuantumClawConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): QuantumClawConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "quantumclaw-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-quantumclaw-extensions/my-plugin",
      },
      extensionsDir,
    });
