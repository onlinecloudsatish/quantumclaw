...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.QUANTUMCLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveQuantumClawPackageRootSync({ cwd: process.cwd() }),
      resolveQuantumClawPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
