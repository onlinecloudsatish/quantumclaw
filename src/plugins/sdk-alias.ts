...
import { resolveQuantumClawPackageRootSync } from "../infra/quantumclaw-root.js";
...
function hasTrustedQuantumClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasQuantumClawBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("quantumclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.quantumclaw === "string");
  const hasQuantumClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "quantumclaw.mjs"));
  return hasCliEntryExport || hasQuantumClawBin || hasQuantumClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedQuantumClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveQuantumClawPackageRootSync({ cwd });
  const fromExplicitHints =
    params.argv1 || params.moduleUrl
      ? resolveQuantumClawPackageRootSync({
          cwd,
...
export function buildPluginLoaderAliasMap(modulePath: string): Record<string, string> {
  ...
  return {
    ...(extensionApiAlias ? { "quantumclaw/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "quantumclaw/plugin-sdk": pluginSdkAlias } : {}),
