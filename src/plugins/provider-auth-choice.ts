...
function restoreConfiguredPrimaryModel(
  ...
  originalConfig: QuantumClawConfig,
): QuantumClawConfig {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: QuantumClawConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveQuantumClawAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
