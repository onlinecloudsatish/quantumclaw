...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-quantumclaw",
      targetDir: "/tmp/opik-quantumclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-quantumclaw": {
              source: "npm",
              spec: "@opik/opik-quantumclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-quantumclaw",
            },
          },
        },
      },
      pluginIds: ["opik-quantumclaw"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-quantumclaw": {
              source: "npm",
              spec: "@opik/opik-quantumclaw@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-quantumclaw",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @quantumclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @quantumclaw/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      targetDir: "/tmp/quantumclaw-codex-app-server",
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "quantumclaw-codex-app-server": {
              source: "npm",
              spec: "quantumclaw-codex-app-server@beta",
              installPath: "/tmp/quantumclaw-codex-app-server",
              resolvedName: "quantumclaw-codex-app-server",
              resolvedSpec: "quantumclaw-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
      pluginIds: ["quantumclaw-codex-app-server"],
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "quantumclaw-codex-app-server@beta",
        expectedPluginId: "quantumclaw-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["quantumclaw-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "quantumclaw-codex-app-server@beta",
      installPath: "/tmp/quantumclaw-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "quantumclaw-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "quantumclaw-codex-app-server@0.2.0-beta.4",
      },
    });
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "quantumclaw-codex-app-server": {
              source: "npm",
              spec: "quantumclaw-codex-app-server",
              installPath: "/tmp/quantumclaw-codex-app-server",
            },
          },
        },
      ...
      pluginIds: ["quantumclaw-codex-app-server"],
      specOverrides: {
        "quantumclaw-codex-app-server": "quantumclaw-codex-app-server@beta",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "quantumclaw-codex-app-server": {
              source: "npm",
              spec: "quantumclaw-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/quantumclaw-codex-app-server",
            },
          },
      ...
      pluginIds: ["quantumclaw-codex-app-server"],
      specOverrides: {
        "quantumclaw-codex-app-server": "quantumclaw-codex-app-server@0.2.0-beta.4",
      },
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "quantumclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@quantumclaw/voice-call",
      targetDir: "/tmp/quantumclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@quantumclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@quantumclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@quantumclaw/voice-call");
    expect(result.config.plugins?.entries?.["@quantumclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@quantumclaw/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@quantumclaw/voice-call",
      installPath: "/tmp/quantumclaw-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/quantumclaw-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
