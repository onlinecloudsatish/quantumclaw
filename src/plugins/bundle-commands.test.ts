...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("quantumclaw-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("quantumclaw-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.QUANTUMCLAW_HOME;
      delete process.env.QUANTUMCLAW_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".quantumclaw", "extensions", "compound-bundle");
