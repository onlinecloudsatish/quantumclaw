...
import { clearPluginDiscoveryCache, discoverQuantumClawPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("quantumclaw-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverQuantumClawPlugins>[0],
) {
  return discoverQuantumClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      quantumclaw: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverQuantumClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".quantumclaw", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".quantumclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverQuantumClawPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@quantumclaw/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@quantumclaw/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@quantumclaw/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@quantumclaw/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@quantumclaw/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("quantumclaw");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@quantumclaw/escape-pack",
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      outsideManifest,
      JSON.stringify({
        name: "@quantumclaw/pack",
        quantumclaw: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverQuantumClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        QUANTUMCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverQuantumClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        QUANTUMCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverQuantumClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        QUANTUMCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
