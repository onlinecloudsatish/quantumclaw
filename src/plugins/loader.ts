...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: QuantumClawPluginDefinition;
  register?: QuantumClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as QuantumClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as QuantumClawPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "quantumclaw",
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.QUANTUMCLAW_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverQuantumClawPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverQuantumClawPlugins>["candidates"][number];
  right: ReturnType<typeof discoverQuantumClawPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadQuantumClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadQuantumClawPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into QuantumClaw yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: QuantumClawPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as QuantumClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
