...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "QUANTUMCLAW_HOME", "QUANTUMCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("quantumclaw-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("quantumclaw-bundle-mcp-workspace-");
      ...
      const config: QuantumClawConfig = {
        plugins: {
          entries: {
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "QUANTUMCLAW_HOME", "QUANTUMCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("quantumclaw-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("quantumclaw-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.QUANTUMCLAW_HOME;
      delete process.env.QUANTUMCLAW_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".quantumclaw", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".quantumclaw", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "QUANTUMCLAW_HOME", "QUANTUMCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("quantumclaw-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "quantumclaw-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.QUANTUMCLAW_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".quantumclaw", "extensions", "inline-claude");
