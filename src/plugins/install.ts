...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing quantumclaw.extensions; update the plugin package to include quantumclaw.extensions (for example ["./dist/index.js"]). See https://docs.quantumclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-quantumclaw-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_QUANTUMCLAW_EXTENSIONS: "missing_quantumclaw_extensions",
  EMPTY_QUANTUMCLAW_EXTENSIONS: "empty_quantumclaw_extensions",
...
function ensureQuantumClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_QUANTUMCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json quantumclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_QUANTUMCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "quantumclaw security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureQuantumClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureQuantumClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from quantumclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-quantumclaw"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json quantumclaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires QuantumClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set QUANTUMCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires QuantumClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade QuantumClaw and retry.`,
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "quantumclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "quantumclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "quantumclaw-npm-pack-",
    spec,
    timeoutMs,
