...
type TempHomeEnvSnapshot = {
  ...
  quantumclawHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    quantumclawHome: process.env.QUANTUMCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("QUANTUMCLAW_HOME", snapshot.quantumclawHome);
  restoreKey("QUANTUMCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.QUANTUMCLAW_STATE_DIR = join(home, ".quantumclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "quantumclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".quantumclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): QuantumClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "quantumclaw"),
