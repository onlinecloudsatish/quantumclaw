...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "quantumclaw"),
    sessionKey,
    remoteCacheDir: join(home, ".quantumclaw", "media", "remote-cache", sessionKey),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("quantumclaw-triggers-", async (home) => {
