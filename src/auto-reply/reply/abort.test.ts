...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop quantumclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@quantumclaw_bot", { botUsername: "quantumclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@quantumclaw_bot", { botUsername: "quantumclaw_bot" })).toBe(true);
