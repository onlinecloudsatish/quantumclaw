...
let originalQuantumClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.QUANTUMCLAW_TEST_FILE_LOG;
  originalQuantumClawLogLevel = process.env.QUANTUMCLAW_LOG_LEVEL;
  delete process.env.QUANTUMCLAW_TEST_FILE_LOG;
  delete process.env.QUANTUMCLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.QUANTUMCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalQuantumClawLogLevel === undefined) {
    delete process.env.QUANTUMCLAW_LOG_LEVEL;
  } else {
    process.env.QUANTUMCLAW_LOG_LEVEL = originalQuantumClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
