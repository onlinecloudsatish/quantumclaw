...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "quantumclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".quantumclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<QuantumClawConfig> = {},
): QuantumClawConfig {
  const base: QuantumClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<QuantumClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<QuantumClawConfig>,
