...
import { buildWebSearchProviderConfig, withTempHome, writeQuantumClawConfig } from "./test-helpers.js";
import { QuantumClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = QuantumClawSchema.safeParse({
      $schema: "https://quantumclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://quantumclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = QuantumClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = QuantumClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeQuantumClawConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeQuantumClawConfig(home, {
        gateway: { bind: "${QUANTUMCLAW_BIND}" },
      });

      const prev = process.env.QUANTUMCLAW_BIND;
      process.env.QUANTUMCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.QUANTUMCLAW_BIND;
        } else {
          process.env.QUANTUMCLAW_BIND = prev;
        }
      }
    });
  });
