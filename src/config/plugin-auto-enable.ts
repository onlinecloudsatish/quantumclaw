...
import type { QuantumClawConfig } from "./config.js";
...
const ENV_CATALOG_PATHS = ["QUANTUMCLAW_PLUGIN_CATALOG_PATHS", "QUANTUMCLAW_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: QuantumClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: QuantumClawConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.quantumclaw) || !isRecord(entry.quantumclaw.channel)) {
      continue;
    }
    const channel = entry.quantumclaw.channel;
...
function collectCandidateChannelIds(cfg: QuantumClawConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: QuantumClawConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: QuantumClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: QuantumClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: QuantumClawConfig, pluginId: string): QuantumClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
