...
function makeTempDir() {
  const dir = mkdtempSafe(path.join(os.tmpdir(), "quantumclaw-plugin-auto-enable-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePluginManifestFixture(params: { rootDir: string; id: string; channels: string[] }) {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "quantumclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function makeRegistry(plugins: Array<{ id: string; channels: string[] }>): PluginManifestRegistry {
  return {
    plugins: plugins.map((p) => ({
      ...
      manifestPath: `/fake/${p.id}/quantumclaw.plugin.json`,
    })),
    diagnostics: [],
  };
}
...
describe("applyPluginAutoEnable", () => {
  ...
  it("uses env-scoped catalog metadata for preferOver auto-enable decisions", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@quantumclaw/env-secondary",
            quantumclaw: {
              channel: {
                id: "env-secondary",
                label: "Env Secondary",
              ...
              install: {
                npmSpec: "@quantumclaw/env-secondary",
              },
            },
          },
        ],
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    it("uses the plugin manifest id, not the channel id, for plugins.entries", () => {
      // Reproduces: https://github.com/quantumclaw/quantumclaw/issues/25261
