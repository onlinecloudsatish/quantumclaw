...
import type { QuantumClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareQuantumClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  quantumclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  quantumclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: QuantumClawConfig,
  pathSegments: string[],
): { changed: boolean; next: QuantumClawConfig } {
...
function coerceConfig(value: unknown): QuantumClawConfig {
...
function stampConfigVersion(cfg: QuantumClawConfig): QuantumClawConfig {
...
function warnIfConfigFromFuture(cfg: QuantumClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareQuantumClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer QuantumClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as QuantumClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): QuantumClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as QuantumClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: QuantumClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as QuantumClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.QUANTUMCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.QUANTUMCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.QUANTUMCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.QUANTUMCLAW_WATCH_SESSION === "string" &&
        deps.env.QUANTUMCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.QUANTUMCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.QUANTUMCLAW_WATCH_COMMAND === "string" &&
        deps.env.QUANTUMCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.QUANTUMCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `QUANTUMCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: QuantumClawConfig | null = null;
let runtimeConfigSourceSnapshot: QuantumClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.QUANTUMCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.QUANTUMCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): QuantumClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): QuantumClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: QuantumClawConfig;
  candidate: QuantumClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: QuantumClawConfig): QuantumClawConfig {
...
export function loadConfig(): QuantumClawConfig {
...
export async function readBestEffortConfig(): Promise<QuantumClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to quantumclaw.json remain visible (no stale snapshot).
}
