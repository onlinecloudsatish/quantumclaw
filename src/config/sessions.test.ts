...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ QUANTUMCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-quantumclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-quantumclaw#general");
  });
  ...
  it("derives session transcripts dir from QUANTUMCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { QUANTUMCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when QUANTUMCLAW_STATE_DIR differs from stored paths", () => {
