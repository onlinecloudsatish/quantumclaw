...
import type { ConfigFileSnapshot } from "./types.quantumclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.quantumclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/quantumclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/quantumclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/quantumclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/quantumclaw/nickserv-password.txt");
