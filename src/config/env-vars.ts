...
function collectConfigEnvVarsByTarget(cfg?: QuantumClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: QuantumClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: QuantumClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: QuantumClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.QUANTUMCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
