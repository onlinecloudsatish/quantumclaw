...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("quantumclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "quantumclaw-sandbox-browser-global" },
      agentBrowser: { network: "quantumclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("quantumclaw-sandbox-browser-agent");
  });
