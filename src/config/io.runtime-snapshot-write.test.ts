...
function createSourceConfig(): QuantumClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): QuantumClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("quantumclaw-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("quantumclaw-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: QuantumClawConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("quantumclaw-config-runtime-write-", async (home) => {
      const configPath = path.join(home, ".quantumclaw", "quantumclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("quantumclaw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".quantumclaw", "quantumclaw.json");
      const sourceConfig: QuantumClawConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: QuantumClawConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("quantumclaw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".quantumclaw", "quantumclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: QuantumClawConfig = {
