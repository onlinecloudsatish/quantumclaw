...
import type { QuantumClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: QuantumClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: QuantumClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: QuantumClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = QuantumClawSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as QuantumClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as QuantumClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as QuantumClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as QuantumClawConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; QuantumClaw mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: QuantumClawConfig | null | undefined;

  const ensureCompatConfig = (): QuantumClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native QuantumClaw config schema.
