...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.QUANTUMCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveQuantumClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    QUANTUMCLAW_STATE_DIR: path.join(os.tmpdir(), "quantumclaw-config-doc-baseline-state"),
    QUANTUMCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.QUANTUMCLAW_CONFIG_DOC_BASELINE_DEBUG === "1"
