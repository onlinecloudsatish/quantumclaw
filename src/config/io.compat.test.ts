...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  ...
  dirname: ".quantumclaw",
  port: number,
  filename: string = "quantumclaw.json",
) {
...
describe("config io paths", () => {
  it("uses ~/.quantumclaw/quantumclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".quantumclaw", 19001);
  ...
  it("defaults to ~/.quantumclaw/quantumclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".quantumclaw", "quantumclaw.json"));
    });
  });

  it("uses QUANTUMCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { QUANTUMCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".quantumclaw", "quantumclaw.json"));
    });
  });

  it("honors explicit QUANTUMCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".quantumclaw", 20002, "custom.json");
      const io = createIoForHome(home, { QUANTUMCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".quantumclaw", 20003, "legacy-custom.json");
