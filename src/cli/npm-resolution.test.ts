...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@quantumclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@quantumclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@quantumclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@quantumclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @quantumclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@quantumclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@quantumclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/quantumclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@quantumclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @quantumclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@quantumclaw/plugin-alpha@latest",
      true,
      "/tmp/quantumclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@quantumclaw/plugin-alpha@latest",
      installPath: "/tmp/quantumclaw/extensions/alpha",
      version: "1.2.3",
