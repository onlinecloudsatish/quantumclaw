...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update QuantumClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["quantumclaw update", "Update a source checkout (git)"],
        ["quantumclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["quantumclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["quantumclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["quantumclaw update --tag main", "One-off package install from GitHub main"],
        ["quantumclaw update --dry-run", "Preview actions without changing anything"],
        ["quantumclaw update --no-restart", "Update without restarting the service"],
        ["quantumclaw update --json", "Output result as JSON"],
        ["quantumclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["quantumclaw update wizard", "Interactive update wizard"],
        ["quantumclaw --update", "Shorthand for quantumclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run quantumclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.quantumclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantumclaw update status", "Show channel + version status."],
          ["quantumclaw update status --json", "JSON output."],
          ["quantumclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.quantumclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
