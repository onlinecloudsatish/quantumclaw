...
import type { QuantumClawConfig, ConfigFileSnapshot } from "../config/types.quantumclaw.js";
...
vi.mock("../infra/quantumclaw-root.js", () => ({
  resolveQuantumClawPackageRoot: vi.fn(),
}));
...
const { resolveQuantumClawPackageRoot } = await import("../infra/quantumclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/quantumclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/quantumclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveQuantumClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("quantumclaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("quantumclaw-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveQuantumClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("quantumclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/quantumclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "quantumclaw-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("QuantumClaw update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as QuantumClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "quantumclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("quantumclaw-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "quantumclaw@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:quantumclaw/quantumclaw#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("quantumclaw-update"));
          await updateCommand({ yes: true, tag: "github:quantumclaw/quantumclaw#main" });
        },
        expectedSpec: "github:quantumclaw/quantumclaw#main",
      },
      {
        name: "QUANTUMCLAW_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("quantumclaw-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/quantumclaw-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("quantumclaw");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("quantumclaw-update");
    const localAppData = createCaseDir("quantumclaw-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            QUANTUMCLAW_STATE_DIR: "./state",
            QUANTUMCLAW_CONFIG_PATH: "./config/quantumclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            QUANTUMCLAW_STATE_DIR: path.resolve("./state"),
            QUANTUMCLAW_CONFIG_PATH: path.resolve("./config/quantumclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            QUANTUMCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ QUANTUMCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.QUANTUMCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("quantumclaw-update-wizard");
    await withEnvAsync({ QUANTUMCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
