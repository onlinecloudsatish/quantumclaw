...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "quantumclaw";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage QuantumClaw plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.quantumclaw.ai/cli/plugins")}\n`,
    );
      ...
      if (!opts.verbose) {
        ...
        const rows = list.map((plugin) => {
          ...
          return {
            ...
            Format: plugin.format ?? "quantumclaw",
            Status:
              plugin.status === "loaded"
                ? theme.success("loaded")
                : plugin.status === "disabled"
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "quantumclaw"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: QuantumClawConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.quantumclaw.ai/plugin");
      lines.push("");
