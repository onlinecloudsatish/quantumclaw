...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantumclaw security audit", "Run a local security audit."],
          ["quantumclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["quantumclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "quantumclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["quantumclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["quantumclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.quantumclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("QuantumClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("quantumclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("quantumclaw security audit --fix")}`));
        if (!fixResult) {
