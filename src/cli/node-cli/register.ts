    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "quantumclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["quantumclaw node status", "Check node host service status."],
          ["quantumclaw node install", "Install the node host service."],
          ["quantumclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.quantumclaw.ai/cli/node")}\n`,
    );
