...
describe("config cli integration", () => {
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-config-cli-int-"));
    const configPath = path.join(tempDir, "quantumclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const envSnapshot = captureEnv([
      "QUANTUMCLAW_CONFIG_PATH",
      "QUANTUMCLAW_TEST_FAST",
      "DISCORD_BOT_TOKEN",
    ]);
    try {
      fs.writeFileSync(
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "quantumclaw.json");
    const envSnapshot = captureEnv([
      "QUANTUMCLAW_CONFIG_PATH",
      "QUANTUMCLAW_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-config-cli-int-exec-skip-"));
    const configPath = path.join(tempDir, "quantumclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["QUANTUMCLAW_CONFIG_PATH", "QUANTUMCLAW_TEST_FAST"]);
    try {
      fs.writeFileSync(
        configPath,
        `${JSON.stringify(
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-config-cli-int-exec-allow-"));
    const configPath = path.join(tempDir, "quantumclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["QUANTUMCLAW_CONFIG_PATH", "QUANTUMCLAW_TEST_FAST"]);
