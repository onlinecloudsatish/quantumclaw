...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.quantumclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + QuantumClaw hooks")
    ...
    .option("--hook-url <url>", "QuantumClaw hook URL")
    .option("--hook-token <token>", "QuantumClaw hook token")
