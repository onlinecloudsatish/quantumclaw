...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-bundled-skills-test-"));
    process.env.QUANTUMCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
