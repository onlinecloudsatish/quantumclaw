    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantumclaw nodes status", "List known nodes with live status."],
          ["quantumclaw nodes pairing pending", "Show pending node pairing requests."],
          ['quantumclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["quantumclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.quantumclaw.ai/cli/nodes")}\n`,
    );
