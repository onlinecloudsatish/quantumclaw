...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("quantumclaw-daemon-install-int-");
    configPath = path.join(tempHome, "quantumclaw.json");
    process.env.HOME = tempHome;
    process.env.QUANTUMCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.QUANTUMCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
