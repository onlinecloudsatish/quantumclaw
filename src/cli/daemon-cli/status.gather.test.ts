...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    QUANTUMCLAW_STATE_DIR: "/tmp/quantumclaw-daemon",
    QUANTUMCLAW_CONFIG_PATH: "/tmp/quantumclaw-daemon/quantumclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.QUANTUMCLAW_STATE_DIR ?? "/tmp/quantumclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.QUANTUMCLAW_CONFIG_PATH ?? `${stateDir}/quantumclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/quantumclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.QUANTUMCLAW_STATE_DIR = "/tmp/quantumclaw-cli";
    process.env.QUANTUMCLAW_CONFIG_PATH = "/tmp/quantumclaw-cli/quantumclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.QUANTUMCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.QUANTUMCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.QUANTUMCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "quantumclaw-gateway" }],
        hints: [],
      },
      healthy: false,
