...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "quantumclaw config set secrets.providers.vault --provider-source file --provider-path /etc/quantumclaw/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("quantumclaw doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: QuantumClawConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as QuantumClawConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "quantumclaw.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.quantumclaw.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing quantumclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
