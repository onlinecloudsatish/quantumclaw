...
import type { ConfigFileSnapshot, QuantumClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `quantumclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: QuantumClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
const mockResolveSecretRefValue = vi.fn();

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: QuantumClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: {
  resolved: QuantumClawConfig;
  config: QuantumClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/quantumclaw.json",
    exists: true,
...
function setSnapshot(resolved: QuantumClawConfig, config: QuantumClawConfig) {
...
function withRuntimeDefaults(resolved: QuantumClawConfig): QuantumClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-quantumclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: QuantumClawConfig = {
        agents: {
      ...
      const runtimeMerged: QuantumClawConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__QUANTUMCLAW_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-quantumclaw.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: QuantumClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("quantumclaw config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "quantumclaw config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/quantumclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.quantumclaw/quantumclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.quantumclaw/quantumclaw.json");
    });
  });
});
