    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantumclaw memory status", "Show index and provider status."],
          ["quantumclaw memory status --deep", "Probe embedding provider readiness."],
          ["quantumclaw memory index --force", "Force a full reindex."],
          ['quantumclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'quantumclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["quantumclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.quantumclaw.ai/cli/memory")}\n`,
    );
