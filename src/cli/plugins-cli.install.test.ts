...
describe("plugins cli install", () => {
  ...
  it("installs marketplace plugins and persists config", async () => {
    ...
    const installedCfg = {
      ...
      plugins: {
        ...
        installs: {
          alpha: {
            source: "marketplace",
            installPath: "/tmp/quantumclaw-state/extensions/alpha",
          },
        },
      },
    } as QuantumClawConfig;
    ...
    installPluginFromMarketplace.mockResolvedValue({
      ok: true,
      pluginId: "alpha",
      targetDir: "/tmp/quantumclaw-state/extensions/alpha",
      version: "1.2.3",
  ...
  it("installs ClawHub plugins and persists source metadata", async () => {
      ...
      plugins: {
        ...
        installs: {
          demo: {
            ...
            installPath: "/tmp/quantumclaw-state/extensions/demo",
            clawhubPackage: "demo",
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "demo",
      targetDir: "/tmp/quantumclaw-state/extensions/demo",
      version: "1.2.3",
      packageName: "demo",
      clawhub: {
        source: "clawhub",
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "quantumclaw skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "quantumclaw skills install demo" instead.');
  });
  ...
  it("falls back to installing hook packs from npm specs", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing quantumclaw.plugin.json",
    });
