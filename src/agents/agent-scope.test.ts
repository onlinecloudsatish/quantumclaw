...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: QuantumClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/quantumclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: QuantumClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: QuantumClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: QuantumClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: QuantumClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: QuantumClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: QuantumClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: QuantumClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: QuantumClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/quantumclaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/quantumclaw");
  });

  it("uses QUANTUMCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "quantumclaw-home");
    vi.stubEnv("QUANTUMCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as QuantumClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".quantumclaw", "workspace"));
  });

  it("uses QUANTUMCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "quantumclaw-home");
    vi.stubEnv("QUANTUMCLAW_HOME", home);
    // Clear state dir so it falls back to QUANTUMCLAW_HOME
    vi.stubEnv("QUANTUMCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as QuantumClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".quantumclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/quantumclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/quantumclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-agent-scope-"));
