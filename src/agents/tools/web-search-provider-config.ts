...
type ConfiguredWebSearchProvider = NonNullable<
  NonNullable<NonNullable<QuantumClawConfig["tools"]>["web"]>["search"]
>["provider"];

export type WebSearchConfig = NonNullable<QuantumClawConfig["tools"]>["web"] extends infer Web
...
export function resolveSearchConfig(cfg?: QuantumClawConfig): WebSearchConfig {
...
export function setProviderWebSearchPluginConfigValue(
  configTarget: QuantumClawConfig,
  pluginId: string,
  key: string,
  value: unknown,
): void {
