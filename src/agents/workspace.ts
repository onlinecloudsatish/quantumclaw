...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.QUANTUMCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".quantumclaw", `workspace-${profile}`);
  }
  return path.join(home, ".quantumclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".quantumclaw";
