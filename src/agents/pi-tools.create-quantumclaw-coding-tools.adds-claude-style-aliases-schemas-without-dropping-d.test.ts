...
describe("createQuantumClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-read-"));
    try {
      ...
      const contents = "Hello from quantumclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createQuantumClawCodingTools({ sandbox });
