...
import { planQuantumClawModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: QuantumClawConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: QuantumClawConfig): {
  config: QuantumClawConfig;
  sourceConfigForSecrets: QuantumClawConfig;
} {
...
export async function ensureQuantumClawModelsJson(
  config?: QuantumClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveQuantumClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planQuantumClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
