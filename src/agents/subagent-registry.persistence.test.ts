...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["QUANTUMCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-subagent-"));
  ...
  it("uses isolated temp state when QUANTUMCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "quantumclaw-test-state"));
  });
});
