...
import { markQuantumClawExecEnv } from "../../infra/quantumclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "quantumclaw.sandbox=1");
  args.push("--label", `quantumclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `quantumclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `quantumclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markQuantumClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "quantumclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`quantumclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`quantumclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("quantumclaw sandbox recreate --all");
}
