  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "quantumclaw.configHash"')
      ) {
...
function createSandboxConfig(
  ...
  return {
    ...
    docker: {
      image: "quantumclaw-sandbox:test",
    ...
    browser: {
      enabled: false,
      image: "quantumclaw-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`quantumclaw.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    expect(createCall).toBeDefined();
    expect(createCall?.args).toContain(`quantumclaw.configHash=${expectedHash}`);
