...
function resolveAttachmentLimits(config: QuantumClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".quantumclaw", "attachments");
  const relDir = path.posix.join(".quantumclaw", "attachments", attachmentId);
