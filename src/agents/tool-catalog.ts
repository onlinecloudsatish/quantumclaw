...
type CoreToolDefinition = {
  ...
  includeInQuantumClawGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "web",
    profiles: ["coding"],
    includeInQuantumClawGroup: true,
  },
  {
    id: "web_fetch",
    label: "web_fetch",
...
function buildCoreToolGroupMap() {
  ...
  const quantumclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInQuantumClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:quantumclaw": quantumclawTools,
