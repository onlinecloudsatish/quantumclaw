...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-tools-pdf-"));
  try {
...
describe("createQuantumClawTools PDF registration", () => {
  it("includes pdf tool when pdfModel is configured", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const tools = createQuantumClawTools({ config: cfg, agentDir });
