...
function mockContextDeps(params: {
  ...
  const ensureQuantumClawModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureQuantumClawModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveQuantumClawAgentDir: () => "/tmp/quantumclaw-agent",
  }));
  ...
  return { ensureQuantumClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("can skip async warmup for read-only callers", async () => {
    const { ensureQuantumClawModelsJson } = mockContextModuleDeps(() => ({
      models: {
        providers: {
          openrouter: {
            models: [{ id: "openrouter/claude-sonnet", contextWindow: 321_000 }],
    ...
    await flushAsyncWarmup();
    expect(ensureQuantumClawModelsJson).not.toHaveBeenCalled();
  });

  it("only warms eagerly for real quantumclaw startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "quantumclaw", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "quantumclaw", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "quantumclaw", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureQuantumClawModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureQuantumClawModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
