...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("quantumclaw-ws-", async (workspaceDir) => {
      await withTempDir("quantumclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createQuantumClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("quantumclaw-ws-", async (workspaceDir) => {
      await withTempDir("quantumclaw-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("quantumclaw-ws-", async (workspaceDir) => {
      const cfg: QuantumClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createQuantumClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-quantumclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("quantumclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("quantumclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createQuantumClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
