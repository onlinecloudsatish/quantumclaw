...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("quantumclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("quantumclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("quantumclaw-sbx-");
