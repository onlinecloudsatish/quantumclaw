...
import { resolvePreferredQuantumClawTmpDir } from "../infra/tmp-quantumclaw-dir.js";
...
describe("resolveSandboxedMediaSource", () => {
  const openClawTmpDir = resolvePreferredQuantumClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred QuantumClaw tmp root",
      media: path.join(openClawTmpDir, "image.png"),
      expected: path.join(openClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred QuantumClaw tmp root",
      media: pathToFileURL(path.join(openClawTmpDir, "photo.png")).href,
      expected: path.join(openClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred QuantumClaw tmp root",
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside quantumclaw tmp root",
      media: path.join(os.tmpdir(), "outside-quantumclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked QuantumClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked QuantumClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQuantumClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked QuantumClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQuantumClawTmp(
