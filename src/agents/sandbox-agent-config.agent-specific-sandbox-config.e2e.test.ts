...
async function resolveContext(config: QuantumClawConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): QuantumClawConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: QuantumClawConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/quantumclaw-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
