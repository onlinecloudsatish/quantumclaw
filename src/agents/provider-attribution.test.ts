...
describe("provider attribution", () => {
  it("resolves the canonical QuantumClaw product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      QUANTUMCLAW_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in QuantumClaw runtime wrapper.",
      product: "QuantumClaw",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { QUANTUMCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      version: "2026.3.22",
      headers: {
        originator: "quantumclaw",
        "User-Agent": "quantumclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { QUANTUMCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "quantumclaw",
      "User-Agent": "quantumclaw/2026.3.22",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { QUANTUMCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ QUANTUMCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
