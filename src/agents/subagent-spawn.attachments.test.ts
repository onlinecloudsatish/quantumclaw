...
let previousConfigPath = process.env.QUANTUMCLAW_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(() => {
    ...
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `quantumclaw-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
    configPathOverride = path.join(workspaceDirOverride, "quantumclaw.test.json");
    fs.writeFileSync(configPathOverride, JSON.stringify(configOverride, null, 2));
    previousConfigPath = process.env.QUANTUMCLAW_CONFIG_PATH;
    process.env.QUANTUMCLAW_CONFIG_PATH = configPathOverride;
  });
  ...
  afterEach(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.QUANTUMCLAW_CONFIG_PATH = previousConfigPath;
    }
    configPathOverride = "";
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".quantumclaw", "attachments");
