...
function redactSessionsSpawnAttachmentsArgs(value: unknown): unknown {
  ...
  const next = raw.map((item) => {
    ...
    return { ...rest, content: "__QUANTUMCLAW_REDACTED__" };
  });
...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[quantumclaw] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
