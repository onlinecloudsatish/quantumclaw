...
let configOverride: QuantumClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: QuantumClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/quantumclaw/quantumclaw/issues/31298
    previousFastTestEnv = process.env.QUANTUMCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.QUANTUMCLAW_TEST_FAST;
      return;
    }
    process.env.QUANTUMCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // QUANTUMCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("QuantumClaw runtime context (internal):");
