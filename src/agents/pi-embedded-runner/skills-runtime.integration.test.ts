...
const originalBundledDir = process.env.QUANTUMCLAW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("quantumclaw-bundled-");
  const workspaceDir = await createTempDir("quantumclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: QuantumClawConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.QUANTUMCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.QUANTUMCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
