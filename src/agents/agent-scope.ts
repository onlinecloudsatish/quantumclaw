...
export function listAgentEntries(cfg: QuantumClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: QuantumClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: QuantumClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: QuantumClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: QuantumClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: QuantumClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
