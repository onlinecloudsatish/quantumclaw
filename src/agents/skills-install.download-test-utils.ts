...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("quantumclaw-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-skills-install-"));
  try {
