...
const GOOGLE_LIVE_SINGLE_KEY = "QUANTUMCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "QUANTUMCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "QUANTUMCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "QUANTUMCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `QUANTUMCLAW_LIVE_${base}_KEY`;
