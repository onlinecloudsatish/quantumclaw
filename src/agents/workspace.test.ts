...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses QUANTUMCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      QUANTUMCLAW_HOME: "/srv/quantumclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/quantumclaw-home"), ".quantumclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".quantumclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("quantumclaw-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("quantumclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".quantumclaw"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-workspace-hardlink-"));
    try {
