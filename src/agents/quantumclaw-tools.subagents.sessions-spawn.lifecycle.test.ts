...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.QUANTUMCLAW_TEST_FAST;
...
describe("quantumclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.QUANTUMCLAW_TEST_FAST = fastModeEnv.previous;
  });
