    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "quantumclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "quantumclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "quantumclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("quantumclaw-");
  const acpxRoot = await tempDirs.make("quantumclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("quantumclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("quantumclaw-");
  const pluginRoot = await tempDirs.make("quantumclaw-plugin-");
  const outsideDir = await tempDirs.make("quantumclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("quantumclaw-");
    const pluginRoot = await tempDirs.make("quantumclaw-claude-bundle-");
