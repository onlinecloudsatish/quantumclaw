...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("quantumclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("quantumclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("quantumclaw-lid-a-", async (first) => {
      await withTempDir("quantumclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.quantumclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "quantumclaw-config-dir-"));
    try {
  ...
  it("expands QUANTUMCLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/quantumclaw-home",
      QUANTUMCLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/quantumclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers QUANTUMCLAW_HOME over HOME", () => {
    vi.stubEnv("QUANTUMCLAW_HOME", "/srv/quantumclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/quantumclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $QUANTUMCLAW_HOME prefix when QUANTUMCLAW_HOME is set", () => {
    vi.stubEnv("QUANTUMCLAW_HOME", "/srv/quantumclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/quantumclaw-home")}/.quantumclaw/quantumclaw.json`)).toBe(
      "$QUANTUMCLAW_HOME/.quantumclaw/quantumclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $QUANTUMCLAW_HOME replacement when QUANTUMCLAW_HOME is set", () => {
    vi.stubEnv("QUANTUMCLAW_HOME", "/srv/quantumclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/quantumclaw-home")}/.quantumclaw/quantumclaw.json`),
    ).toBe("config: $QUANTUMCLAW_HOME/.quantumclaw/quantumclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/quantumclaw", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "quantumclaw"),
    );
  });
  ...
  it("prefers QUANTUMCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("QUANTUMCLAW_HOME", "/srv/quantumclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/quantumclaw")).toBe(path.resolve("/srv/quantumclaw-home", "quantumclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/quantumclaw", env)).toBe(path.resolve("/srv/quantumclaw-home", "quantumclaw"));
  });
