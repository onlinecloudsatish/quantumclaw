...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): QuantumClawConfig {
    const defaults: NonNullable<NonNullable<QuantumClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: QuantumClawConfig) {
