...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".quantumclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.QUANTUMCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$QUANTUMCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via QUANTUMCLAW_STATE_DIR.
