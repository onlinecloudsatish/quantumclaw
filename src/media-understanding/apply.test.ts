...
const TEMP_MEDIA_PREFIX = "quantumclaw-media-";
...
function createGroqAudioConfig(): QuantumClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): QuantumClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): QuantumClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: QuantumClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredQuantumClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        QUANTUMCLAW_AGENT_DIR: isolatedAgentDir,
