...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `quantumclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      QUANTUMCLAW_LAUNCHD_LABEL: `ai.quantumclaw.launchd-int-${testId}`,
      QUANTUMCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
