...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when QUANTUMCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".quantumclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when QUANTUMCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", QUANTUMCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".quantumclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers QUANTUMCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        QUANTUMCLAW_PROFILE: "rescue",
        QUANTUMCLAW_STATE_DIR: "C:\\State\\quantumclaw",
      },
      expected: path.join("C:\\State\\quantumclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", QUANTUMCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".quantumclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem QuantumClaw Gateway",
          "cd /d C:\\Projects\\quantumclaw",
          "set NODE_ENV=production",
          "set QUANTUMCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\quantumclaw",
          environment: {
            NODE_ENV: "production",
            QUANTUMCLAW_PORT: "18789",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\quantumclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\quantumclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\QuantumClaw Share\\node.exe" "\\\\fileserver\\QuantumClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\QuantumClaw Share\\node.exe",
            "\\\\fileserver\\QuantumClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from QUANTUMCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ QUANTUMCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
