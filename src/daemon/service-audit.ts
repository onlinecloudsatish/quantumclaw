...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds QUANTUMCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `quantumclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service QUANTUMCLAW_GATEWAY_TOKEN does not match gateway.auth.token in quantumclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.QUANTUMCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.QUANTUMCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `quantumclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
