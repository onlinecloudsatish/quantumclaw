...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.quantumclaw.gateway";
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.quantumclaw.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", QUANTUMCLAW_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("uses the configured gateway port for stale cleanup", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      QUANTUMCLAW_GATEWAY_PORT: "19001",
    };
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.quantumclaw.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when QUANTUMCLAW_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.quantumclaw.gateway.plist",
    },
    {
      name: "uses profile-specific label when QUANTUMCLAW_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", QUANTUMCLAW_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.quantumclaw.jbphoenix.plist",
    },
    {
      name: "prefers QUANTUMCLAW_LAUNCHD_LABEL over QUANTUMCLAW_PROFILE",
      env: {
        HOME: "/Users/test",
        QUANTUMCLAW_PROFILE: "jbphoenix",
        QUANTUMCLAW_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from QUANTUMCLAW_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        QUANTUMCLAW_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty QUANTUMCLAW_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        QUANTUMCLAW_PROFILE: "myprofile",
        QUANTUMCLAW_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.quantumclaw.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
