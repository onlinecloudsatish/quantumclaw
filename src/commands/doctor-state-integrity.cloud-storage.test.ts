...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "QuantumClaw", ".quantumclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/quantumclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".quantumclaw");
  ...
  it("anchors cloud detection to OS homedir when QUANTUMCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".quantumclaw");
    const originalQuantumClawHome = process.env.QUANTUMCLAW_HOME;
    process.env.QUANTUMCLAW_HOME = "/tmp/quantumclaw-home-override";
    ...
    try {
      ...
      if (originalQuantumClawHome === undefined) {
