...
const SEARCH_TOOL = "https://docs.quantumclaw.ai/mcp.SearchQuantumClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.quantumclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('quantumclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.quantumclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('quantumclaw docs "your query"')}`);
    }
    return;
  }
