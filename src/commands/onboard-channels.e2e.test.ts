...
function createTelegramCfg(botToken: string, enabled?: boolean): QuantumClawConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelSetupWizardAdapter>[1]) {
  return patchChannelSetupWizardAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: QuantumClawConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as QuantumClawConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: QuantumClawConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as QuantumClawConfig, prompter, {
      quickstartDefaults: true,
  ...
  it("renders the QuickStart channel picker without requiring the LINE runtime", async () => {
    ...
    await expect(
      runSetupChannels({} as QuantumClawConfig, prompter, {
        quickstartDefaults: true,
      }),
    ).resolves.toEqual({} as QuantumClawConfig);
  ...
  it("keeps configured external plugin channels visible when the active registry starts empty", async () => {
    ...
    catalogMocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "msteams",
        pluginId: "@quantumclaw/msteams-plugin",
        meta: {
          id: "msteams",
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@quantumclaw/msteams-plugin": { enabled: true },
          },
        },
      } as QuantumClawConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              capabilities: { chatTypes: ["direct"] },
              config: {
                listAccountIds: (cfg: QuantumClawConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: QuantumClawConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
              ...
              setupWizard: {
                ...
                status: {
                  ...
                  resolveConfigured: ({ cfg }: { cfg: QuantumClawConfig }) =>
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: QuantumClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: QuantumClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
