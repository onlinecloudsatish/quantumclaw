...
beforeAll(() => {
  envSnapshot = captureEnv(["QUANTUMCLAW_PROFILE"]);
  process.env.QUANTUMCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/quantumclaw-root.js", () => ({
  resolveQuantumClawPackageRoot: vi.fn().mockResolvedValue("/tmp/quantumclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/quantumclaw/pnpm-lock.yaml",
      markerPath: "/tmp/quantumclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.quantumclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.quantumclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("quantumclaw status --all") ||
          line.includes("quantumclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("QUANTUMCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
