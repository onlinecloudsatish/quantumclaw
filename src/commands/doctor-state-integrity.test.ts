...
type EnvSnapshot = {
  HOME?: string;
  QUANTUMCLAW_HOME?: string;
  QUANTUMCLAW_STATE_DIR?: string;
  QUANTUMCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    QUANTUMCLAW_HOME: process.env.QUANTUMCLAW_HOME,
    QUANTUMCLAW_STATE_DIR: process.env.QUANTUMCLAW_STATE_DIR,
    QUANTUMCLAW_OAUTH_DIR: process.env.QUANTUMCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: QuantumClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: QuantumClawConfig) {
...
async function runStateIntegrityText(cfg: QuantumClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.QUANTUMCLAW_HOME = tempHome;
    process.env.QUANTUMCLAW_STATE_DIR = path.join(tempHome, ".quantumclaw");
    delete process.env.QUANTUMCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.QUANTUMCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when QUANTUMCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.QUANTUMCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: QuantumClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints quantumclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: QuantumClawConfig = {};
    ...
    expect(text).toMatch(/quantumclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/quantumclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /quantumclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
