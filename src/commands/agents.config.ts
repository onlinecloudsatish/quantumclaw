...
function resolveAgentName(cfg: QuantumClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: QuantumClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: QuantumClawConfig): AgentSummary[] {
