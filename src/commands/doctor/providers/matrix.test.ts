...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "quantumclaw-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("quantumclaw plugins install @quantumclaw/matrix");
    expect(warnings[2]).toContain("quantumclaw plugins install ./extensions/matrix");
  });
