...
const ensureQuantumClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveQuantumClawAgentDir = vi.fn().mockReturnValue("/tmp/quantumclaw-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/quantumclaw-state",
  loadConfig,
...
beforeEach(() => {
  ...
  ensureQuantumClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureQuantumClawModelsJson).toHaveBeenCalled();
    expect(ensureQuantumClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
