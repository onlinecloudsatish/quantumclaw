...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.quantumclaw/quantumclaw.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.quantumclaw/workspace",
  applyWizardMetadata: (cfg: QuantumClawConfig) => cfg,
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.applySearchProviderSelection.mockImplementation((cfg: QuantumClawConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by applySearchKey", async () => {
    ...
    mocks.applySearchKey.mockImplementation(
      (cfg: QuantumClawConfig, provider: string, key: string) => ({
        ...cfg,
        tools: {
          ...cfg.tools,
          web: {
  ...
  it("applies provider selection side effects when a key already exists via secret ref or env", async () => {
    ...
    mocks.applySearchProviderSelection.mockImplementation(
      (cfg: QuantumClawConfig, provider: string) => ({
        ...cfg,
        tools: {
          ...cfg.tools,
          web: {
