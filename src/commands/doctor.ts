...
import { resolveQuantumClawPackageRoot } from "../infra/quantumclaw-root.js";
...
function resolveMode(cfg: QuantumClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("QuantumClaw doctor");

  const root = await resolveQuantumClawPackageRoot({
  ...
  let cfg: QuantumClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("quantumclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("quantumclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("quantumclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("quantumclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("quantumclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("quantumclaw doctor --fix")}" to apply changes.`);
  }
