...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.QUANTUMCLAW_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: QuantumClawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: QuantumClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.QUANTUMCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
