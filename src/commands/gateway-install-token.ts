...
import { readConfigFileSnapshot, writeConfigFile, type QuantumClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("quantumclaw config set gateway.auth.mode token")} or ${formatCliCommand("quantumclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
