...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.QUANTUMCLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("quantumclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
