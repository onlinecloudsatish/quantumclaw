...
import { QuantumClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: QuantumClawConfig): {
  config: QuantumClawConfig;
  removed: string[];
} {
  const parsed = QuantumClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: QuantumClawConfig): void {
