...
import type { QuantumClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: QuantumClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: QuantumClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("quantumclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.quantumclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("quantumclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.quantumclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("quantumclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("quantumclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("quantumclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("quantumclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('quantumclaw config set session.dmScope "per-channel-peer"') +
