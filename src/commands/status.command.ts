...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByQuantumClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByQuantumClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("QuantumClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`quantumclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("quantumclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("quantumclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("quantumclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("quantumclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.quantumclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.quantumclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("quantumclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("quantumclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("quantumclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("quantumclaw gateway probe")}`);
  }
}
