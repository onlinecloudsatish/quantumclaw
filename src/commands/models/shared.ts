...
export async function loadValidConfigOrThrow(): Promise<QuantumClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: QuantumClawConfig) => QuantumClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: QuantumClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: QuantumClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("quantumclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
