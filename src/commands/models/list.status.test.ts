...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveQuantumClawAgentDir: vi.fn().mockReturnValue("/tmp/quantumclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/quantumclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/quantumclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/quantumclaw-dev/quantumclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveQuantumClawAgentDir: mocks.resolveQuantumClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/quantumclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveQuantumClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/quantumclaw-dev/quantumclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/quantumclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/quantumclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/quantumclaw-agent-custom");
