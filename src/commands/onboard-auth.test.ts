...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "QUANTUMCLAW_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "QUANTUMCLAW_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under QUANTUMCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("quantumclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-oauth-sync-"));
    ...
    process.env.QUANTUMCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-oauth-nosync-"));
    process.env.QUANTUMCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside QUANTUMCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-oauth-external-"));
    process.env.QUANTUMCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* QUANTUMCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to QUANTUMCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("quantumclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
