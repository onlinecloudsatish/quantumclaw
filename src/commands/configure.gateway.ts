...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "QUANTUMCLAW_GATEWAY_TOKEN",
          placeholder: "QUANTUMCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like QUANTUMCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. QuantumClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: QuantumClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.quantumclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
