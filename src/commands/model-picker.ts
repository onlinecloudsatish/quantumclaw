...
type PromptDefaultModelResult = { model?: string; config?: QuantumClawConfig };
...
function resolveConfiguredModelRaw(cfg: QuantumClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: QuantumClawConfig): string[] {
...
export function applyModelAllowlist(cfg: QuantumClawConfig, models: string[]): QuantumClawConfig {
