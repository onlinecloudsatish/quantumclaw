...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: QuantumClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: QuantumClawConfig) => { changed: boolean; next: QuantumClawConfig };
  defaultModel: string;
  overrideConfig: QuantumClawConfig;
  alreadyDefaultConfig: QuantumClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: QuantumClawConfig) => config,
      applyDefaultConfig: (config: QuantumClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: QuantumClawConfig = { agents: { defaults: {} } };
