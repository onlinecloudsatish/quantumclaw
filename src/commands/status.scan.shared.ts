...
export function hasExplicitMemorySearchConfig(cfg: QuantumClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: QuantumClawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: QuantumClawConfig, agentId: string) => { store: { path: string } } | null;
