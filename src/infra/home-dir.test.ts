...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers QUANTUMCLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for QUANTUMCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { QUANTUMCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when QUANTUMCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ QUANTUMCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores QUANTUMCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          QUANTUMCLAW_HOME: "/srv/quantumclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { QUANTUMCLAW_HOME: "/srv/quantumclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/quantumclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/quantumclaw-home " },
      expected: "/srv/quantumclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/quantumclaw-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of QUANTUMCLAW_HOME", () => {
    expect(
