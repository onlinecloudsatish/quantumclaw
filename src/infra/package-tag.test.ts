...
describe("normalizePackageTagInput", () => {
  const packageNames = ["quantumclaw", "@quantumclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("quantumclaw@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@quantumclaw/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("quantumclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("quantumclaw", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @quantumclaw/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("quantumclawer@beta", packageNames)).toBe("quantumclawer@beta");
  });
});
