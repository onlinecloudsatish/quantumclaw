...
describe("matrix migration snapshots", () => {
  ...
  it("creates a backup marker after writing a pre-migration snapshot", async () => {
    await withTempHome(async (home) => {
      const archivePath = path.join(home, "Backups", "quantumclaw-migrations", "snapshot.tar.gz");
      fs.mkdirSync(path.dirname(archivePath), { recursive: true });
      fs.writeFileSync(path.join(home, ".quantumclaw", "quantumclaw.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(home, ".quantumclaw", "state.txt"), "state\n", "utf8");
    ...
    await withTempHome(async (home) => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          version: 1,
          createdAt: "2026-03-10T18:00:00.000Z",
          archivePath: path.join(home, "Backups", "quantumclaw-migrations", "missing.tar.gz"),
          trigger: "older-run",
          includeWorkspace: false,
        }),
        "utf8",
  ...
  it("does not treat warning-only Matrix migration as actionable", async () => {
    await withTempHome(async (home) => {
      const stateDir = path.join(home, ".quantumclaw");
  ...
  it("treats legacy Matrix crypto as warning-only until the plugin helper is available", async () => {
    await withTempHome(
      ...
      {
        env: {
          QUANTUMCLAW_BUNDLED_PLUGINS_DIR: (home) => path.join(home, "empty-bundled"),
        },
      },
    );
  });
