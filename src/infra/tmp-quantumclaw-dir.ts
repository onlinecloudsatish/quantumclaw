...
export const POSIX_QUANTUMCLAW_TMP_DIR = "/tmp/quantumclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredQuantumClawTmpDirOptions = {
...
export function resolvePreferredQuantumClawTmpDir(
  options: ResolvePreferredQuantumClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "quantumclaw" : `quantumclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[quantumclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback QuantumClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback QuantumClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_QUANTUMCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_QUANTUMCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_QUANTUMCLAW_TMP_DIR)) {
      return POSIX_QUANTUMCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_QUANTUMCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_QUANTUMCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_QUANTUMCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_QUANTUMCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_QUANTUMCLAW_TMP_DIR;
