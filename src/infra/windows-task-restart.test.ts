...
const resolvePreferredQuantumClawTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-quantumclaw-dir.js", () => ({
  resolvePreferredQuantumClawTmpDir: () => resolvePreferredQuantumClawTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredQuantumClawTmpDirMock.mockReset();
  resolvePreferredQuantumClawTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ QUANTUMCLAW_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "QuantumClaw Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "QuantumClaw Gateway (work)" >nul 2>&1');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers QUANTUMCLAW_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    relaunchGatewayScheduledTask({
      QUANTUMCLAW_PROFILE: "work",
      QUANTUMCLAW_WINDOWS_TASK_NAME: "QuantumClaw Gateway (custom)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "QuantumClaw Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredQuantumClawTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ QUANTUMCLAW_PROFILE: "work" });
