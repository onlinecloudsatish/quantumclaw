...
const DEFAULT_PACKAGE_NAME = "quantumclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["quantumclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "quantumclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("quantumclaw doctor", doctorArgv, gitRoot, { QUANTUMCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
