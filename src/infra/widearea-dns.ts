...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.QUANTUMCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*quantumclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "quantumclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "quantumclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "quantumclaw-gw");
  ...
  records.push(`_quantumclaw-gw._tcp IN PTR ${instanceLabel}._quantumclaw-gw._tcp`);
  records.push(`${instanceLabel}._quantumclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._quantumclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; quantumclaw-content-hash: ${contentHash}\n${contentBody}`;
}
