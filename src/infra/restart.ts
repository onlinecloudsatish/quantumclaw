...
export function triggerQuantumClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.QUANTUMCLAW_SYSTEMD_UNIT,
      process.env.QUANTUMCLAW_PROFILE,
    );
  ...
  const label =
    process.env.QUANTUMCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.QUANTUMCLAW_PROFILE);
