...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.quantumclaw.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ QUANTUMCLAW_WINDOWS_TASK_NAME: "QuantumClaw Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
    ...
    expect(
      detectRespawnSupervisor(
        {
          QUANTUMCLAW_SERVICE_MARKER: "quantumclaw",
          QUANTUMCLAW_SERVICE_KIND: "worker",
        },
        "win32",
      ),
    ).toBeNull();
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.quantumclaw.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
