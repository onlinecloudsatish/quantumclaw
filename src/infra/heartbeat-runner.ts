...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: QuantumClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: QuantumClawConfig) {
...
function resolveHeartbeatAgents(cfg: QuantumClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: QuantumClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: QuantumClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: QuantumClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
