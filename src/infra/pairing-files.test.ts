...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/quantumclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/quantumclaw-state", "devices"),
      pendingPath: path.join("/tmp/quantumclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/quantumclaw-state", "devices", "paired.json"),
    });
  });
