...
let ensureQuantumClawCliOnPath: typeof import("./path-env.js").ensureQuantumClawCliOnPath;

describe("ensureQuantumClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "QUANTUMCLAW_PATH_BOOTSTRAPPED",
    "QUANTUMCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureQuantumClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/quantumclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "quantumclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureQuantumClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling quantumclaw exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.QUANTUMCLAW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.QUANTUMCLAW_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "quantumclaw");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.QUANTUMCLAW_PATH_BOOTSTRAPPED;
      delete process.env.QUANTUMCLAW_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.QUANTUMCLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/quantumclaw-path/case-linuxbrew");
