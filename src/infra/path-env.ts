...
type EnsureQuantumClawPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureQuantumClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `quantumclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "quantumclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.QUANTUMCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "quantumclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `quantumclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureQuantumClawCliOnPath(opts: EnsureQuantumClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.QUANTUMCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
