...
const GATEWAY_DEVICE_ID_HEADER = "x-quantumclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-quantumclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-quantumclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.QUANTUMCLAW_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "QUANTUMCLAW_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.QUANTUMCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require QUANTUMCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.QUANTUMCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
