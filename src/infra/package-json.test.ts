...
describe("package-json helpers", () => {
  it("reads package version and trims package name", async () => {
    await withTempDir({ prefix: "quantumclaw-package-json-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: " 1.2.3 ", name: "  @quantumclaw/demo  " }),
        "utf8",
      );

      await expect(readPackageVersion(root)).resolves.toBe("1.2.3");
      await expect(readPackageName(root)).resolves.toBe("@quantumclaw/demo");
    });
  });
  ...
  it("returns null for missing or invalid package.json data", async () => {
    await withTempDir({ prefix: "quantumclaw-package-json-" }, async (root) => {
      ...
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: "   ", name: "@quantumclaw/demo" }),
        "utf8",
      );
      await expect(readPackageVersion(root)).resolves.toBeNull();
    });
