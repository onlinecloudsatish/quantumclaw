...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred QuantumClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideQuantumClaw = path.join(os.tmpdir(), "outside-quantumclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
            params: {
              channel: "slack",
              target: "#C12345678",
              media: hostTmpOutsideQuantumClaw,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
