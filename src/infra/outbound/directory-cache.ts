...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: QuantumClawConfig | null = null;
  ...
  get(key: string, cfg: QuantumClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: QuantumClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: QuantumClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: QuantumClawConfig): void {
