...
import { resolvePreferredQuantumClawTmpDir } from "../tmp-quantumclaw-dir.js";
...
const telegramChunkConfig: QuantumClawConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: QuantumClawConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes QuantumClaw tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredQuantumClawTmpDir()]),
      }),
    );
  });

  it("includes QuantumClaw tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as QuantumClawConfig,
      channel: "matrix",
  ...
  it("includes QuantumClaw tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes QuantumClaw tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: QuantumClawConfig = { channels: { signal: { mediaMaxMb: 2 } } };
