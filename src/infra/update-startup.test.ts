...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["QUANTUMCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.QUANTUMCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveQuantumClawPackageRoot } = await import("./quantumclaw-root.js"));
    ...
    vi.mocked(resolveQuantumClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveQuantumClawPackageRoot).mockResolvedValue("/opt/quantumclaw");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/quantumclaw",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/quantumclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
