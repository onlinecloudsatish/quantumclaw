...
describe("restart sentinel", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["QUANTUMCLAW_STATE_DIR"]);
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantumclaw-sentinel-"));
  ...
  it("formats summary, distinct reason, and doctor hint together", () => {
    const payload = {
      ...
      doctorHint: "Run quantumclaw doctor",
