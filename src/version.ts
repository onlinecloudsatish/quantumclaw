...
declare const __QUANTUMCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "quantumclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["QUANTUMCLAW_VERSION"],
      runtimeVersion,
      env["QUANTUMCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current QuantumClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __QUANTUMCLAW_VERSION__ === "string" ? __QUANTUMCLAW_VERSION__ : undefined,
  bundledVersion: process.env.QUANTUMCLAW_BUNDLED_VERSION,
});
