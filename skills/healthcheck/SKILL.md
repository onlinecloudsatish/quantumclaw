---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for QuantumClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, QuantumClaw cron scheduling for periodic checks, or version status checks on a machine running QuantumClaw (laptop, workstation, Pi, VPS).
---

# QuantumClaw Host Hardening

## Overview

Assess and harden the host running QuantumClaw, then align it to a user-defined risk tolerance without breaking access. Use QuantumClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim QuantumClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. QuantumClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if QuantumClaw can access sensitive data.
...
### 2) Run QuantumClaw security audits (read-only)

As part of the default read-only checks, run `quantumclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `quantumclaw security audit` (faster, non-probing)
2. `quantumclaw security audit --json` (structured output)

Offer to apply QuantumClaw safe defaults (numbered):

1. `quantumclaw security audit --fix`

Be explicit that `--fix` only tightens QuantumClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check QuantumClaw version/update status (read-only)

As part of the default read-only checks, run `quantumclaw update status`.
...
- Credential hygiene notes (location of QuantumClaw creds, prefer disk encryption)
...
- QuantumClaw security audit (re-run)
...
After QuantumClaw install or first hardening pass, run at least one baseline audit and version check:

- `quantumclaw security audit`
- `quantumclaw security audit --deep`
- `quantumclaw update status`

Ongoing monitoring is recommended. Use the QuantumClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `quantumclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `quantumclaw update status`
...
Before creating, `quantumclaw cron list` and match on exact `name`. If found, `quantumclaw cron edit <id> ...`.
If not found, `quantumclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `quantumclaw update status` (preferred for source checkouts and channels)
2. `npm view quantumclaw version` (published npm version)

## QuantumClaw command accuracy

Use only supported commands and flags:

- `quantumclaw security audit [--deep] [--fix] [--json]`
- `quantumclaw status` / `quantumclaw status --deep`
- `quantumclaw health --json`
- `quantumclaw update status`
- `quantumclaw cron add|list|runs|run`

Do not invent CLI flags or imply QuantumClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by QuantumClaw compaction:
