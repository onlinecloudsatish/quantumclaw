---
name: node-connect
description: Diagnose QuantumClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify QuantumClaw is advertising that route, then fix pairing/auth.
...
- whether `quantumclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `quantumclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
quantumclaw config get gateway.mode
quantumclaw config get gateway.bind
quantumclaw config get gateway.tailscale.mode
quantumclaw config get gateway.remote.url
quantumclaw config get gateway.auth.mode
quantumclaw config get gateway.auth.allowTailscale
quantumclaw config get plugins.entries.device-pair.config.publicUrl
quantumclaw qr --json
quantumclaw devices list
quantumclaw nodes status
```

If this QuantumClaw instance is pointed at a remote gateway, also run:

```bash
quantumclaw qr --remote --json
```
...
`quantumclaw qr --json` success means:
...
If `quantumclaw qr --json` says `Gateway is only bound to loopback`:
...
- `quantumclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run quantumclaw qr again, rescan, then approve the pending device pairing.`
