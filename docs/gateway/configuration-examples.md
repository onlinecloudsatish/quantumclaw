---
summary: "Schema-accurate configuration examples for common QuantumClaw setups"
read_when:
  - Learning how to configure QuantumClaw
...
```json5
{
  agent: { workspace: "~/.quantumclaw/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.quantumclaw/quantumclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.quantumclaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.quantumclaw/cron/cron.json",
    maxConcurrentRuns: 2,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/quantumclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
