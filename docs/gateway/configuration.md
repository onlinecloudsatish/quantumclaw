...
read_when:
  - Setting up QuantumClaw for the first time
...
QuantumClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.quantumclaw/quantumclaw.json`.

If the file is missing, QuantumClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `quantumclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    quantumclaw onboard       # full onboarding flow
    quantumclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.quantumclaw/quantumclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
QuantumClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`quantumclaw doctor`, `quantumclaw logs`, `quantumclaw health`, `quantumclaw status`)
- Run `quantumclaw doctor` to see exact issues
- Run `quantumclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `quantumclaw.json`.
    ...
    ```bash
    quantumclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `QUANTUMCLAW_APNS_RELAY_BASE_URL` and `QUANTUMCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `QUANTUMCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.quantumclaw/quantumclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `quantumclaw config set` for single keys.
    </Warning>
    ...
    ```bash
    quantumclaw gateway call config.get --params '{}'  # capture payload.hash
    quantumclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.quantumclaw/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    quantumclaw gateway call config.patch --params '{
...
QuantumClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.quantumclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, QuantumClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `QUANTUMCLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${QUANTUMCLAW_GATEWAY_TOKEN}" } },
