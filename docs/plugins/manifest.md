...
read_when:
  - You are building an QuantumClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (quantumclaw.plugin.json)

This page is for the **native QuantumClaw plugin manifest** only.
...
QuantumClaw auto-detects those bundle layouts too, but they are not validated
against the `quantumclaw.plugin.json` schema described here.

For compatible bundles, QuantumClaw currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches QuantumClaw runtime expectations.

Every native QuantumClaw plugin **must** ship a `quantumclaw.plugin.json` file in the
**plugin root**. QuantumClaw uses this manifest to validate configuration
...
`quantumclaw.plugin.json` is the metadata QuantumClaw reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that QuantumClaw can inspect without loading plugin code.                                      |
...
QuantumClaw reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, QuantumClaw falls back to `choiceId`.                                        |
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `quantumclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `quantumclaw` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if QuantumClaw must know it before loading plugin code, put it in `quantumclaw.plugin.json`
...
- The manifest is **required for native QuantumClaw plugins**, including local filesystem loads.
