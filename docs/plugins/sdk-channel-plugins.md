...
summary: "Step-by-step guide to building a messaging channel plugin for QuantumClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect QuantumClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects QuantumClaw to a
...
Channel plugins do not need their own send/edit/react tools. QuantumClaw keeps one
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/quantumclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "quantumclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect QuantumClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { QuantumClawConfig } from "quantumclaw/plugin-sdk/core";
  ...
  <Step title="Add a setup entry">
    ...
    QuantumClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    QuantumClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to QuantumClaw.
...
├── package.json              # quantumclaw.channel metadata
├── quantumclaw.plugin.json      # Manifest with config schema
