---
summary: "Install and use Codex, Claude, and Cursor bundles as QuantumClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how QuantumClaw maps bundle content into native features
...
QuantumClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
QuantumClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native QuantumClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native QuantumClaw plugins, QuantumClaw
...
## What QuantumClaw maps from bundles

Not every bundle feature runs in QuantumClaw today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal QuantumClaw skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | QuantumClaw-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but QuantumClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit QuantumClaw best when they use skill roots and QuantumClaw-style
...
QuantumClaw checks for native plugin format first:

1. `quantumclaw.plugin.json` or valid `package.json` with `quantumclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, QuantumClaw uses the native path. This prevents
...
- QuantumClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `quantumclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. QuantumClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    QuantumClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
