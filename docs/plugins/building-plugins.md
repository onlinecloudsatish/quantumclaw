...
summary: "Create your first QuantumClaw plugin in minutes"
read_when:
  - You want to create a new QuantumClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to QuantumClaw
---

# Building Plugins

Plugins extend QuantumClaw with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the QuantumClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`quantumclaw plugins install <package-name>`. QuantumClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect QuantumClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/quantumclaw-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "quantumclaw": {
        "extensions": ["./index.ts"]
    ...
    ```json quantumclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to QuantumClaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** publish to [ClawHub](/tools/clawhub) or npm, then install:
    ...
    QuantumClaw checks ClawHub first, then falls back to npm.
...
Always import from focused `quantumclaw/plugin-sdk/<subpath>` paths:
...
import { ... } from "quantumclaw/plugin-sdk";
```
...
<Check>**package.json** has correct `quantumclaw` metadata</Check>
<Check>**quantumclaw.plugin.json** manifest is present and valid</Check>
