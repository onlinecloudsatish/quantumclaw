...
read_when:
  - You want to use Groq with QuantumClaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. QuantumClaw connects
...
media-understanding provider, QuantumClaw uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.quantumclaw/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `quantumclaw models list | grep groq`
