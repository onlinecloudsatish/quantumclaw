---
summary: "Use OpenAI via API keys or Codex subscription in QuantumClaw"
read_when:
  - You want to use OpenAI models in QuantumClaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like QuantumClaw.
...
OpenAI API usage. QuantumClaw forwards both through the `openai/*` Responses path.
QuantumClaw intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

QuantumClaw does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in QuantumClaw.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. QuantumClaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, QuantumClaw also supports:

- `openai-codex/gpt-5.3-codex-spark`

QuantumClaw treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

QuantumClaw also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
QuantumClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), QuantumClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. QuantumClaw enables it by default for
...
QuantumClaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
QuantumClaw exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, QuantumClaw applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), QuantumClaw now auto-enables OpenAI server-side
