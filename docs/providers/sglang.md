---
summary: "Run QuantumClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run QuantumClaw against a local SGLang server
...
QuantumClaw can connect to SGLang using the `openai-completions` API.

QuantumClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, QuantumClaw will query:
