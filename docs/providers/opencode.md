---
summary: "Use OpenCode Zen and Go catalogs with QuantumClaw"
read_when:
...
OpenCode exposes two hosted catalogs in QuantumClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. QuantumClaw keeps the runtime provider ids
...
quantumclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
quantumclaw onboard --auth-choice opencode-go
quantumclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
