---
summary: "Use MiniMax models in QuantumClaw"
read_when:
  - You want MiniMax models in QuantumClaw
...
QuantumClaw's MiniMax provider defaults to **MiniMax M2.7** and keeps
...
```bash
quantumclaw plugins enable minimax  # skip if already loaded.
quantumclaw gateway restart  # restart if gateway is already running
quantumclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/quantumclaw/quantumclaw/tree/main/extensions/minimax) for details.
...
Configure manually via `quantumclaw.json`:
...
## Configure via `quantumclaw configure`

Use the interactive config wizard to set MiniMax without editing JSON:

1. Run `quantumclaw configure`.
2. Select **Model/auth**.
...
- Use `quantumclaw models list` and `quantumclaw models set minimax/MiniMax-M2.7` to switch.
...
- Running `quantumclaw configure` and selecting a **MiniMax** auth option, or
