---
summary: "Use Venice AI privacy-focused models in QuantumClaw"
read_when:
  - You want privacy-focused inference in QuantumClaw
...
After setup, QuantumClaw shows all available Venice models. Pick based on your needs:
...
QuantumClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `quantumclaw models list` to see currently available models. Some models may be temporarily offline.
