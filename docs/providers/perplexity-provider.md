...
```bash
quantumclaw configure --section web
```

Or set it directly:

```bash
quantumclaw config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
```
...
`~/.quantumclaw/.env` or via `env.shellEnv`).
