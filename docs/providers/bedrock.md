---
summary: "Use Amazon Bedrock (Converse API) models with QuantumClaw"
read_when:
  - You want to use Amazon Bedrock models with QuantumClaw
...
QuantumClaw can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, QuantumClaw can automatically discover Bedrock
...
When running QuantumClaw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, QuantumClaw's credential detection currently only checks for environment
...
- QuantumClaw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
