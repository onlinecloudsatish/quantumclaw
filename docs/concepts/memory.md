---
title: "Memory"
summary: "How QuantumClaw memory works (workspace files + automatic memory flush)"
read_when:
...
QuantumClaw memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, QuantumClaw only loads `MEMORY.md`.
  - Lowercase `memory.md` is only used as a fallback when `MEMORY.md` is absent.
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.quantumclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

QuantumClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, QuantumClaw triggers a **silent,
...
QuantumClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
