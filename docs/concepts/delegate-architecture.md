---
summary: "Delegate architecture: running QuantumClaw as a named agent on behalf of an organization"
...
Goal: run QuantumClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an QuantumClaw agent that:
...
QuantumClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of QuantumClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.quantumclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.quantumclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.quantumclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through QuantumClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
quantumclaw agents add delegate
```

This creates:

- Workspace: `~/.quantumclaw/workspace-delegate`
- State: `~/.quantumclaw/agents/delegate/agent`
- Sessions: `~/.quantumclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.quantumclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.quantumclaw/workspace-delegate",
        tools: {
...
~/.quantumclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.quantumclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.quantumclaw/workspace-org",
        agentDir: "~/.quantumclaw/agents/org-assistant/agent",
