...
QuantumClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models QuantumClaw can use (plus aliases).
...
`quantumclaw models` (no subcommand) is a shortcut for `models status`.
...
`quantumclaw models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.quantumclaw/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: QuantumClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever QuantumClaw regenerates `models.json`, including command-driven paths like `quantumclaw agent`.
