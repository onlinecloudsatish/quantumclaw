...
QuantumClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `quantumclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
QuantumClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
