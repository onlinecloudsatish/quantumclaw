---
summary: "ClawHub guide: public registry, native QuantumClaw install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **QuantumClaw skills and plugins**.

- Use native `quantumclaw` commands to search/install/update skills and install
...
Native `quantumclaw` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for QuantumClaw skills.
...
4. Users browse, download, and install skills in QuantumClaw.
...
If you want to add new capabilities to your QuantumClaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `quantumclaw skills install <skill-slug>`
3. Start a new QuantumClaw session so it picks up the new skill.
...
## How it fits into QuantumClaw

Native `quantumclaw skills install` installs into the active workspace `skills/`
directory. `quantumclaw plugins install clawhub:...` records a normal managed
...
current working directory. If an QuantumClaw workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). QuantumClaw loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.quantumclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches QuantumClaw how to perform a
...
Interested in becoming a moderator? Ask in the QuantumClaw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to QuantumClaw workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/quantumclaw/skills` and `~/.quantumclaw/skills`). This is designed to find older skill installs without extra flags.
