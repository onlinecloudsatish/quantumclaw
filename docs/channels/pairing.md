...
read_when:
  ...
  - Reviewing QuantumClaw security posture
title: "Pairing"
---

# Pairing

“Pairing” is QuantumClaw’s explicit **owner approval** step.
...
Stored under `~/.quantumclaw/credentials/`:
...
3. On your phone, open the QuantumClaw iOS app → Settings → Gateway.
...
Stored under `~/.quantumclaw/devices/`:
...
- The legacy `node.pair.*` API (CLI: `quantumclaw nodes pending/approve`) is a
