...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure QuantumClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `quantumclaw pairing approve slack <code>`.
...
- The interactive callback values are QuantumClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, QuantumClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while QuantumClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```bash
quantumclaw pairing list slack
```
...
QuantumClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, QuantumClaw falls back to normal delivery for remaining payloads.
