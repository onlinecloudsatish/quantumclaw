...
read_when:
  - Setting up Twitch chat integration for QuantumClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. QuantumClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
quantumclaw plugins install @quantumclaw/twitch
```

Local checkout (when running from a git repo):

```bash
quantumclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `QUANTUMCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "quantumclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use QUANTUMCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
QUANTUMCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
