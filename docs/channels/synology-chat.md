---
summary: "Synology Chat webhook setup and QuantumClaw config"
read_when:
  - Setting up Synology Chat with QuantumClaw
...
1. Install and enable the Synology Chat plugin.
   - `quantumclaw onboard` now shows Synology Chat in the same channel setup list as `quantumclaw channels add`.
   - Non-interactive setup: `quantumclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your QuantumClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in QuantumClaw.
   - Guided: `quantumclaw onboard`
   - Direct: `quantumclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
