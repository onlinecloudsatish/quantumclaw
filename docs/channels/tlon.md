...
Tlon is a decentralized messenger built on Urbit. QuantumClaw connects to your Urbit ship and can
...
```bash
quantumclaw plugins install @quantumclaw/tlon
```

Local checkout (when running from a git repo):

```bash
quantumclaw plugins install ./extensions/tlon
```
...
By default, QuantumClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `quantumclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, QuantumClaw replies in-thread.
