---
summary: "Mattermost bot setup and QuantumClaw config"
read_when:
...
```bash
quantumclaw plugins install @quantumclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
quantumclaw plugins install ./extensions/mattermost
```
...
4. Configure QuantumClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, QuantumClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, QuantumClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as QuantumClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to QuantumClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from QuantumClaw, not `404`.
...
- Approve via:
  - `quantumclaw pairing list mattermost`
  - `quantumclaw pairing approve mattermost <CODE>`
...
Use these target formats with `quantumclaw message send` or cron/webhooks:
...
QuantumClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), QuantumClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When QuantumClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, QuantumClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and QuantumClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="quantumclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"quantumclaw-mattermost-interactions",
...
`quantumclaw message send` and cron/webhook deliveries.
