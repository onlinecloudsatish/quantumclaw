...
This guide summarizes a sane workflow for working on the pi integration in QuantumClaw.
...
```bash
QUANTUMCLAW_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm quantumclaw agent --message "Hello" --thinking low`
...
State lives under the QuantumClaw state directory. Default is `~/.quantumclaw`. If `QUANTUMCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `quantumclaw.json` for config
