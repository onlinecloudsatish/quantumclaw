...
CLI onboarding is the **recommended** way to set up QuantumClaw on macOS,
...
`quantumclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`quantumclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.quantumclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `quantumclaw doctor` first.
</Note>
...
Use `quantumclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.quantumclaw/workspace-<agentId>`.
...
- CLI command reference: [`quantumclaw onboard`](/cli/onboard)
