---
summary: "Deploy QuantumClaw on Railway with one-click template"
read_when:
  - Deploying QuantumClaw to Railway
  - You want a one-click cloud deploy with browser-based setup
title: "Railway"
---

Deploy QuantumClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/quantumclaw` — Control UI
...
- `QUANTUMCLAW_STATE_DIR=/data/.quantumclaw` (recommended)
- `QUANTUMCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `QUANTUMCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your QuantumClaw state + workspace so you can migrate to another host without losing config or memory.
