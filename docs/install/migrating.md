---
summary: "Move (migrate) an QuantumClaw install from one machine to another"
read_when:
  - You are moving QuantumClaw to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating QuantumClaw to a New Machine

This guide moves an QuantumClaw gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.quantumclaw/` by default) and your **workspace**, you preserve:

- **Config** -- `quantumclaw.json` and all gateway settings
...
<Tip>
Run `quantumclaw status` on the old machine to confirm your state directory path.
Custom profiles use `~/.quantumclaw-<profile>/` or a path set via `QUANTUMCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    If you use multiple profiles (e.g. `~/.quantumclaw-work`), archive each separately.

  </Step>

  <Step title="Install QuantumClaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.quantumclaw/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `QUANTUMCLAW_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `quantumclaw doctor`.
  </Accordion>

  <Accordion title="Copying only quantumclaw.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-quantumclaw-store-its-data).
  </Accordion>
...
- [ ] `quantumclaw status` shows the gateway running
