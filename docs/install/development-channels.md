...
QuantumClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/quantumclaw`, override with
  `QUANTUMCLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
quantumclaw update --tag 2026.3.22
...
quantumclaw update --tag quantumclaw@2026.3.22
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `quantumclaw update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  QuantumClaw prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `quantumclaw update` would do without making changes:

```bash
quantumclaw update --dry-run
quantumclaw update --channel beta --dry-run
quantumclaw update --tag 2026.3.22 --dry-run
quantumclaw update --dry-run --json
```
...
When you switch channels with `quantumclaw update`, QuantumClaw also syncs plugin
sources:
