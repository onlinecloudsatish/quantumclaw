---
summary: "Deploy QuantumClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run QuantumClaw on a Kubernetes cluster
  - You want to test QuantumClaw in a Kubernetes environment
title: "Kubernetes"
---

# QuantumClaw on Kubernetes

A minimal starting point for running QuantumClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

QuantumClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/quantumclaw 18789:18789 -n quantumclaw
...
```bash
kubectl get secret quantumclaw-secrets -n quantumclaw -o jsonpath='{.data.QUANTUMCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: quantumclaw (configurable via QUANTUMCLAW_NAMESPACE)
├── Deployment/quantumclaw        # Single pod, init container + gateway
├── Service/quantumclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/quantumclaw-config  # quantumclaw.json + AGENTS.md
└── Secret/quantumclaw-secrets    # Gateway token + API keys
```
...
Edit `quantumclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret quantumclaw-secrets -n quantumclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/quantumclaw -n quantumclaw
```

### Custom namespace

```bash
QUANTUMCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/quantumclaw/quantumclaw:latest # or pin to a specific version from https://github.com/quantumclaw/quantumclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # quantumclaw.json + AGENTS.md
