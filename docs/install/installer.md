...
<Note>
If install succeeds but `quantumclaw` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    Checks Node version and installs Node 24 if needed (Homebrew on macOS, NodeSource setup scripts on Linux apt/dnf/yum). QuantumClaw still supports Node 22 LTS, currently `22.16+`, for compatibility.
  </Step>
  ...
  <Step title="Install QuantumClaw">
    ...
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/quantumclaw`
  </Step>
  <Step title="Post-install tasks">
    - Runs `quantumclaw doctor --non-interactive` on upgrades and git installs (best effort)
...
If run inside an QuantumClaw checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
| `QUANTUMCLAW_GIT_UPDATE=0\|1`                              | Toggle git updates                            |
| `QUANTUMCLAW_NO_PROMPT=1`                                  | Disable prompts                               |
...
<Info>
Designed for environments where you want everything under a local prefix (default `~/.quantumclaw`) and no system Node dependency.
</Info>
...
<Steps>
  ...
  <Step title="Install QuantumClaw under prefix">
    Installs with npm using `--prefix <prefix>`, then writes wrapper to `<prefix>/bin/quantumclaw`.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Install QuantumClaw">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\quantumclaw.cmd`
  </Step>
  <Step title="Post-install tasks">
    Adds needed bin directory to user PATH when possible, then runs `quantumclaw doctor --non-interactive` on upgrades and git installs (best effort).
  </Step>
</Steps>
...
| `QUANTUMCLAW_GIT_DIR=<path>`          | Checkout directory |
| `QUANTUMCLAW_NO_ONBOARD=1`            | Skip onboarding    |
| `QUANTUMCLAW_GIT_UPDATE=0`            | Disable git pull   |
| `QUANTUMCLAW_DRY_RUN=1`               | Dry run mode       |
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "quantumclaw is not recognized"'>
  ...
  <Accordion title="quantumclaw not found after install">
