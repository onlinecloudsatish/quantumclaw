---
summary: "Optional Docker-based setup and onboarding for QuantumClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run QuantumClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export QUANTUMCLAW_IMAGE="ghcr.io/quantumclaw/quantumclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/quantumclaw/quantumclaw/pkgs/container/quantumclaw).
...
<Note>
Run `docker compose` from the repo root. If you enabled `QUANTUMCLAW_EXTRA_MOUNTS`
or `QUANTUMCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `QUANTUMCLAW_IMAGE`               | Use a remote image instead of building locally                   |
| `QUANTUMCLAW_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `QUANTUMCLAW_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `QUANTUMCLAW_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `QUANTUMCLAW_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `QUANTUMCLAW_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `QUANTUMCLAW_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `QUANTUMCLAW_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `QUANTUMCLAW_CONFIG_DIR` to `/home/node/.quantumclaw` and
`QUANTUMCLAW_WORKSPACE_DIR` to `/home/node/.quantumclaw/workspace`, so those paths
...
and rolling file logs under `/tmp/quantumclaw/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/quantumclaw/quantumclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/quantumclaw/quantumclaw/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export QUANTUMCLAW_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export QUANTUMCLAW_SANDBOX=1
    export QUANTUMCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm quantumclaw-cli gateway probe
    docker compose run -T --rm quantumclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `quantumclaw-cli` uses `network_mode: "service:quantumclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `quantumclaw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.quantumclaw`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export QUANTUMCLAW_HOME_VOLUME="quantumclaw_home"`
    2. **Bake system deps**: `export QUANTUMCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `QUANTUMCLAW_HOME_VOLUME` or `QUANTUMCLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/quantumclaw/quantumclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    QuantumClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm quantumclaw-cli config set gateway.mode local
    docker compose run --rm quantumclaw-cli config set gateway.bind lan
    docker compose run --rm quantumclaw-cli devices list --url ws://127.0.0.1:18789
    ```
