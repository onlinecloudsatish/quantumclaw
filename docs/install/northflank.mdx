---
summary: "Deploy QuantumClaw on Northflank with one-click template"
read_when:
  - Deploying QuantumClaw to Northflank
  - You want a one-click cloud deploy with browser-based setup
title: "Northflank"
---

Deploy QuantumClaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy QuantumClaw](https://northflank.com/stacks/deploy-quantumclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy QuantumClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the QuantumClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the QuantumClaw service.
8. Open the public QuantumClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/quantumclaw`.

## What you get

- Hosted QuantumClaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/quantumclaw`
