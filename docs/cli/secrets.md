---
summary: "CLI reference for `quantumclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `quantumclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
quantumclaw secrets reload
quantumclaw secrets reload --json
```
...
Scan QuantumClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `quantumclaw.json` refs)
...
```bash
quantumclaw secrets audit
quantumclaw secrets audit --check
quantumclaw secrets audit --json
quantumclaw secrets audit --allow-exec
```
...
```bash
quantumclaw secrets configure
quantumclaw secrets configure --plan-out /tmp/quantumclaw-secrets-plan.json
quantumclaw secrets configure --apply --yes
quantumclaw secrets configure --providers-only
quantumclaw secrets configure --skip-provider-setup
quantumclaw secrets configure --agent ops
quantumclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `quantumclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, QuantumClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
quantumclaw secrets apply --from /tmp/quantumclaw-secrets-plan.json --json
```
...
- `quantumclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.quantumclaw/.env` known secret keys whose values were migrated
