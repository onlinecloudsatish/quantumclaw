---
summary: "CLI reference for `quantumclaw message` (send + channel actions)"
read_when:
...
```
quantumclaw message <subcommand> [flags]
```
...
- On cache miss, QuantumClaw will attempt a live directory lookup when the provider supports it.
...
- `quantumclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, QuantumClaw does not force a `default` account SecretRef scope
...
```
quantumclaw message send --channel discord \
...
```
quantumclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
quantumclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
quantumclaw message poll --channel msteams \
...
```
quantumclaw message react --channel slack \
...
```
quantumclaw message react --channel signal \
...
```
quantumclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
quantumclaw message send --channel telegram --target @mychat \
