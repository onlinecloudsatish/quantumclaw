---
summary: "CLI reference for `quantumclaw onboard` (interactive onboarding)"
read_when:
...
```bash
quantumclaw onboard
quantumclaw onboard --flow quickstart
quantumclaw onboard --flow manual
quantumclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`QUANTUMCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `quantumclaw gateway run`.
...
- Fastest first chat: `quantumclaw dashboard` (Control UI, no channel setup).
