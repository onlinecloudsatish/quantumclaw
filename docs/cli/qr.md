---
summary: "CLI reference for `quantumclaw qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
quantumclaw qr
quantumclaw qr --setup-code-only
quantumclaw qr --json
quantumclaw qr --remote
quantumclaw qr --url wss://gateway.example/ws
```
