---
summary: "QuantumClaw on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up QuantumClaw on Oracle Cloud
  - Looking for low-cost VPS hosting for QuantumClaw
  - Want 24/7 QuantumClaw on a small server
title: "Oracle Cloud"
---

# QuantumClaw on Oracle Cloud (OCI)

## Goal

Run a persistent QuantumClaw Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for QuantumClaw (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh quantumclaw` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@quantumclaw` (or use the Tailscale IP).

## 5) Install QuantumClaw
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `quantumclaw security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.quantumclaw`
- **Security audit:** `quantumclaw security audit`
...
# Re-authenticate
sudo tailscale up --ssh --hostname=quantumclaw --reset
```
...
journalctl --user -u quantumclaw-gateway -n 50
```
...
- `~/.quantumclaw/workspace/` — workspace (SOUL.md, memory, artifacts)
