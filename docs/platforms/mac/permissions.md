...
- Same path: run the app from a fixed location (for QuantumClaw, `dist/QuantumClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.quantumclaw.mac
sudo tccutil reset ScreenCapture ai.quantumclaw.mac
...
Workaround: move files into the QuantumClaw workspace (`~/.quantumclaw/workspace`) if you want to avoid per-folder grants.
