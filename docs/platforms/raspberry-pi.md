---
summary: "QuantumClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
...
# QuantumClaw on Raspberry Pi

## Goal

Run a persistent, always-on QuantumClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
## 9) Access the QuantumClaw Dashboard
...
(or `QUANTUMCLAW_GATEWAY_TOKEN`) into Control UI settings.
...
- `QUANTUMCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running QuantumClaw, add a service drop-in to reduce restart
...
If possible, keep QuantumClaw state/cache on SSD-backed storage to avoid SD-card
...
Most QuantumClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled quantumclaw
...
```bash
# Check logs
journalctl -u quantumclaw --no-pager -n 100

# Common fix: rebuild
cd ~/quantumclaw  # if using hackable install
npm run build
sudo systemctl restart quantumclaw
```
