...
QuantumClaw features that can generate provider usage or paid API calls.
...
- `quantumclaw status --usage` and `quantumclaw channels list` show provider **usage windows**
...
QuantumClaw can pick up credentials from:
...
- `quantumclaw status --usage`
- `quantumclaw models status --json`
...
`quantumclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
