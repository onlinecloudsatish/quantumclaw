...
- `pnpm test` on Node 22, 23, and 24 uses Vitest `vmForks` by default for local runs with enough memory. CI stays on `forks` unless explicitly overridden. Node 25+ falls back to `forks` until re-validated. You can force behavior with `QUANTUMCLAW_TEST_VM_FORKS=0|1`.
...
- Gateway integration: opt-in via `QUANTUMCLAW_TEST_INCLUDE_GATEWAY=1 pnpm test` or `pnpm test:gateway`.
- `pnpm test:e2e`: Runs gateway end-to-end smoke tests (multi-instance WS/HTTP/node pairing). Defaults to `vmForks` + adaptive workers in `vitest.e2e.config.ts`; tune with `QUANTUMCLAW_E2E_WORKERS=<n>` and set `QUANTUMCLAW_E2E_VERBOSE=1` for verbose logs.
...
Script: [`scripts/bench-model.ts`](https://github.com/quantumclaw/quantumclaw/blob/main/scripts/bench-model.ts)
...
Script: [`scripts/bench-cli-startup.ts`](https://github.com/quantumclaw/quantumclaw/blob/main/scripts/bench-cli-startup.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `quantumclaw health`.
