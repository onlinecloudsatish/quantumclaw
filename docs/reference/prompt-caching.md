...
- With Anthropic API-key auth profiles, QuantumClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, QuantumClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
QuantumClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.quantumclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$QUANTUMCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `QUANTUMCLAW_CACHE_TRACE=1` enables cache tracing.
- `QUANTUMCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `QUANTUMCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `QUANTUMCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `QUANTUMCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
