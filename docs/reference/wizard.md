...
This is the full reference for `quantumclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.quantumclaw/quantumclaw.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `quantumclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.quantumclaw/credentials/oauth.json`; auth profiles live in `~/.quantumclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.quantumclaw/credentials/oauth.json` (or `$QUANTUMCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.quantumclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `quantumclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `quantumclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `quantumclaw health`.
    - Tip: `quantumclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export QUANTUMCLAW_GATEWAY_TOKEN="your-token"
quantumclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env QUANTUMCLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.quantumclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.quantumclaw/quantumclaw.json`:
...
`quantumclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.quantumclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.quantumclaw/agents/<agentId>/sessions/`.
