...
```bash
quantumclaw plugins install @quantumclaw/nextcloud-talk
```

本地检出安装（从 git 仓库运行时）：

```bash
quantumclaw plugins install ./extensions/nextcloud-talk
```
...
2. 在你的 Nextcloud 服务器上创建机器人：
   ```bash
   ./occ talk:bot:install "QuantumClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
3. 在目标房间设置中启用机器人。
4. 配置 QuantumClaw：
...
- 批准方式：
  - `quantumclaw pairing list nextcloud-talk`
  - `quantumclaw pairing approve nextcloud-talk <CODE>`
