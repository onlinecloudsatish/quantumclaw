...
summary: 在一台主机上运行多个 QuantumClaw Gateway 网关（隔离、端口和配置档案）
title: 多个 Gateway 网关
x-i18n:
...
- `QUANTUMCLAW_CONFIG_PATH` — 每个实例单独的配置文件
- `QUANTUMCLAW_STATE_DIR` — 每个实例单独的会话、凭证、缓存
...
配置档案会自动限定 `QUANTUMCLAW_STATE_DIR` + `QUANTUMCLAW_CONFIG_PATH`，并为服务名称添加后缀。
