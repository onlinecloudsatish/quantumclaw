...
- 默认滚动日志文件位于 `/tmp/quantumclaw/` 下（每天一个文件）：`quantumclaw-YYYY-MM-DD.log`
  - 日期使用 Gateway 网关主机的本地时区。
- 日志文件路径和级别可以通过 `~/.quantumclaw/quantumclaw.json` 配置：
  - `logging.file`
  - `logging.level`
...
`quantumclaw gateway` 支持每个 Gateway 网关的样式切换：
