...
你可以在 `~/.quantumclaw/quantumclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.quantumclaw/quantumclaw.json` 的 `logging` 下。
...
- QuantumClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `quantumclaw plugins enable diagnostics-otel` 启用插件。
...
- `quantumclaw.tokens`（计数器，属性：`quantumclaw.token`、`quantumclaw.channel`、`quantumclaw.provider`、`quantumclaw.model`）
- `quantumclaw.cost.usd`（计数器，属性：`quantumclaw.channel`、`quantumclaw.provider`、`quantumclaw.model`）
- `quantumclaw.run.duration_ms`（直方图，属性：`quantumclaw.channel`、`quantumclaw.provider`、`quantumclaw.model`）
- `quantumclaw.context.tokens`（直方图，属性：`quantumclaw.context`、`quantumclaw.channel`、`quantumclaw.provider`、`quantumclaw.model`）

消息流：

- `quantumclaw.webhook.received`（计数器，属性：`quantumclaw.channel`、`quantumclaw.webhook`）
- `quantumclaw.webhook.error`（计数器，属性：`quantumclaw.channel`、`quantumclaw.webhook`）
- `quantumclaw.webhook.duration_ms`（直方图，属性：`quantumclaw.channel`、`quantumclaw.webhook`）
- `quantumclaw.message.queued`（计数器，属性：`quantumclaw.channel`、`quantumclaw.source`）
- `quantumclaw.message.processed`（计数器，属性：`quantumclaw.channel`、`quantumclaw.outcome`）
- `quantumclaw.message.duration_ms`（直方图，属性：`quantumclaw.channel`、`quantumclaw.outcome`）

队列 + 会话：

- `quantumclaw.queue.lane.enqueue`（计数器，属性：`quantumclaw.lane`）
- `quantumclaw.queue.lane.dequeue`（计数器，属性：`quantumclaw.lane`）
- `quantumclaw.queue.depth`（直方图，属性：`quantumclaw.lane` 或 `quantumclaw.channel=heartbeat`）
- `quantumclaw.queue.wait_ms`（直方图，属性：`quantumclaw.lane`）
- `quantumclaw.session.state`（计数器，属性：`quantumclaw.state`、`quantumclaw.reason`）
- `quantumclaw.session.stuck`（计数器，属性：`quantumclaw.state`）
- `quantumclaw.session.stuck_age_ms`（直方图，属性：`quantumclaw.state`）
- `quantumclaw.run.attempt`（计数器，属性：`quantumclaw.attempt`）
...
- `quantumclaw.webhook.error`
  - `quantumclaw.channel`、`quantumclaw.webhook`、`quantumclaw.chatId`、`quantumclaw.error`
- `quantumclaw.message.processed`
  - `quantumclaw.channel`、`quantumclaw.outcome`、`quantumclaw.chatId`、`quantumclaw.messageId`、`quantumclaw.sessionKey`、`quantumclaw.sessionId`、`quantumclaw.reason`
- `quantumclaw.session.stuck`
  - `quantumclaw.state`、`quantumclaw.ageMs`、`quantumclaw.queueDepth`、`quantumclaw.sessionKey`、`quantumclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `quantumclaw doctor`。
