...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`quantumclaw hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/.quantumclaw/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `quantumclaw hooks list` 中显示 `plugin:<id>`，
...
```bash
# 本地目录
quantumclaw hooks install ./my-hook-pack

# 本地压缩包
quantumclaw hooks install ./my-hook-pack.zip

# NPM 包
quantumclaw hooks install @quantumclaw/my-hook-pack

# 链接本地目录而不复制
quantumclaw hooks install -l ./my-hook-pack
```

## 更新钩子

```bash
quantumclaw hooks update <id>
quantumclaw hooks update --all
```
...
**输出：** `~/.quantumclaw/workspace/memory/YYYY-MM-DD-slug.md`
