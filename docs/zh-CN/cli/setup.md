...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`quantumclaw setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.quantumclaw/quantumclaw.json` 和智能体工作区。
...
```bash
quantumclaw setup
quantumclaw setup --workspace ~/.quantumclaw/workspace
```

通过 setup 运行新手引导：

```bash
quantumclaw setup --wizard
```
