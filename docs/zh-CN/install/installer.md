...
<Note>
如果安装成功但在新终端中找不到 `quantumclaw`，请参见 [Node.js 故障排除](/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="默认确保使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux apt/dnf/yum 上使用 NodeSource 设置脚本）。为了兼容性，QuantumClaw 仍支持 Node 22 LTS，目前为 `22.16+`。
  </Step>
  <Step title="确保安装 Git">
    如果缺少 Git，则安装它。
  </Step>
  <Step title="安装 QuantumClaw">
    - `npm` 方法（默认）：全局 npm 安装
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，构建，然后将包装器安装到 `~/.local/bin/quantumclaw`
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `quantumclaw doctor --non-interactive`（尽力而为）
...
如果在 QuantumClaw 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
| `QUANTUMCLAW_GIT_UPDATE=0\|1`                              | 切换 git 更新                        |
| `QUANTUMCLAW_NO_PROMPT=1`                                  | 禁用提示                             |
...
<Info>
适用于你希望所有内容都放在本地前缀（默认 `~/.quantumclaw`）下，并且不依赖系统 Node 的环境。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 QuantumClaw">
    使用 `--prefix <prefix>` 通过 npm 安装，然后将包装器写入 `<prefix>/bin/quantumclaw`。
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="安装 QuantumClaw">
    ...
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装/构建，并将包装器安装到 `%USERPROFILE%\.local\bin\quantumclaw.cmd`
  </Step>
  <Step title="安装后任务">
    在可能情况下将所需 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `quantumclaw doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
| `QUANTUMCLAW_GIT_DIR=<path>`          | 检出目录      |
| `QUANTUMCLAW_NO_ONBOARD=1`            | 跳过新手引导  |
| `QUANTUMCLAW_GIT_UPDATE=0`            | 禁用 git pull |
| `QUANTUMCLAW_DRY_RUN=1`               | 试运行模式    |
...
<AccordionGroup>
  ...
  <Accordion title="安装后找不到 quantumclaw">
