...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("quantumclaw-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("quantumclaw-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("quantumclaw-openshell-remote-agent-");
