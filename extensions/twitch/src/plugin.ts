/**
 * Twitch channel plugin for QuantumClaw.
 *
 ...
 * for QuantumClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: QuantumClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: QuantumClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: QuantumClawConfig): boolean => {
  ...
  resolver: {
    resolveTargets: async ({
      ...
      runtime: import("quantumclaw/plugin-sdk/runtime-env").RuntimeEnv;
