...
function getChannelConfig(cfg: QuantumClawConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: QuantumClawConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: QuantumClawConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: QuantumClawConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL QuantumClaw uses to send replies back to Chat.",
      ],
