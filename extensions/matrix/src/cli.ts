...
import { isQuantumClawManagedMatrixDevice } from "./matrix/device-health.js";
import {
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `quantumclaw matrix ${command}${suffix}`;
}
...
type MatrixCliAccountAddResult = {
  ...
  deviceHealth: {
    ...
    staleQuantumClawDeviceIds: string[];
    error?: string;
  };
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleQuantumClawDeviceIds: addedDevices
        .filter((device) => !device.current && isQuantumClawManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.quantumclaw.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleQuantumClawDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale QuantumClaw devices detected (${result.deviceHealth.staleQuantumClawDeviceIds.join(", ")}). Run 'quantumclaw matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `quantumclaw agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale QuantumClaw-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale QuantumClaw devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
