...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleQuantumClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale QuantumClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleQuantumClawDevices.map((device) => device.deviceId).join(", ")}. Run 'quantumclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
