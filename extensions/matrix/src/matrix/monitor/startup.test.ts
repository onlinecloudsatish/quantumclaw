...
const hoisted = vi.hoisted(() => ({
  ...
  summarizeMatrixDeviceHealth: vi.fn(() => ({
    staleQuantumClawDevices: [] as Array<{ deviceId: string }>,
  })),
...
describe("runMatrixStartupMaintenance", () => {
  beforeEach(() => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockClear().mockReturnValue({ staleQuantumClawDevices: [] });
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockReturnValue({
      staleQuantumClawDevices: [{ deviceId: "DEV123" }],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale QuantumClaw devices detected for @bot:example.org: DEV123. Run 'quantumclaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'quantumclaw matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
