...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another QuantumClaw runtime is using the same account. " +
    `Check 'quantumclaw matrix verify status --verbose --account ${accountId}' and 'quantumclaw matrix devices list --account ${accountId}'.`
  );
}
