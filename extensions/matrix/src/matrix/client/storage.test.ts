...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-quantumclaw-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-matrix-storage-"));
    const stateDir = path.join(homeDir, ".quantumclaw");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      QUANTUMCLAW_HOME: path.dirname(stateDir),
      QUANTUMCLAW_STATE_DIR: stateDir,
      QUANTUMCLAW_TEST_FAST: "1",
