...
import { resetInboundDedupe } from "quantumclaw/plugin-sdk/reply-runtime";
import type { MsgContext } from "quantumclaw/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "quantumclaw/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "quantumclaw/plugin-sdk/reply-runtime";
import type { MockFn } from "quantumclaw/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("quantumclaw/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("quantumclaw/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("quantumclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/quantumclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("quantumclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("quantumclaw/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("quantumclaw/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("quantumclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("quantumclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
const replySpyHoisted = vi.hoisted(() => ({
  replySpy: vi.fn(async (_ctx: MsgContext, opts?: GetReplyOptions) => {
    ...
    (
      ...
      configOverride?: QuantumClawConfig,
...
function createModelsProviderDataFromConfig(cfg: QuantumClawConfig): {
...
vi.doMock("quantumclaw/plugin-sdk/command-auth", async (importOriginal) => {
...
vi.doMock("quantumclaw/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("quantumclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("quantumclaw/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("quantumclaw/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: QuantumClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: QuantumClawConfig) => {
