...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): QuantumClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as QuantumClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): QuantumClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as QuantumClawConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: QuantumClawConfig, accountId = "media") {
    await handleTelegramAction(
