...
import { normalizeSecretInput } from "quantumclaw/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<QuantumClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<QuantumClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: QuantumClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: QuantumClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: QuantumClawConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: QuantumClawConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: QuantumClawConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: QuantumClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: QuantumClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: QuantumClawConfig, override?: number): number {
