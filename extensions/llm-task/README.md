...
`quantumclaw.invoke --each`) without adding new QuantumClaw code per workflow.
...
This extension depends on QuantumClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** QuantumClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.quantumclaw/extensions` as a standalone plugin directory.
