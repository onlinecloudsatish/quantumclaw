import type { AnyAgentTool, QuantumClawPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: QuantumClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<QuantumClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as QuantumClawPluginApi,
    resolveTool,
  };
}
