...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-quantumclaw-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-quantumclaw-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
<html lang="en">
  ...
  <body data-theme="${params.theme}">
    <main class="oc-frame" data-render-mode="${params.runtimeMode}">
      <div data-quantumclaw-diff-root>
        ${params.bodyHtml}
      </div>
    </main>
