...
import type { QuantumClawPluginApi, QuantumClawPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<QuantumClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: QuantumClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<QuantumClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as QuantumClawPluginApi);
