...
Read-only diff viewer plugin for **QuantumClaw** agents.
...
Set plugin-wide defaults in `~/.quantumclaw/quantumclaw.json`:
...
Before:
QuantumClaw supports plugins.

After:
QuantumClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/quantumclaw-diffs`).
