import type { SlackSlashCommandConfig } from "quantumclaw/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "quantumclaw");
  const name = normalizedName || "quantumclaw";
  return {
