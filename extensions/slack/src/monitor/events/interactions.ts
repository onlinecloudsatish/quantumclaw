...
// Prefix for QuantumClaw-generated action IDs to scope our handler
const QUANTUMCLAW_ACTION_PREFIX = "quantumclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${QUANTUMCLAW_ACTION_PREFIX}`);

  // Handle QuantumClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
