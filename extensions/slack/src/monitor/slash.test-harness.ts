...
vi.mock("quantumclaw/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/routing")>();
  return {
    ...actual,
...
export function resetSlackSlashMocks() {
  ...
  mocks.resolveStorePathMock.mockReset().mockReturnValue("/tmp/quantumclaw-sessions.json");
}
