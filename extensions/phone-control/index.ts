...
function normalizeDenyList(cfg: QuantumClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: QuantumClawPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: QuantumClawPluginApi) {
    ...
    const timerService: QuantumClawPluginService = {
