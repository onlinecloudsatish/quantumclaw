...
metadata: { "quantumclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == QuantumClaw `sessions_spawn`
- **File I/O** == QuantumClaw `read`/`write`
- **Remote fetch** == QuantumClaw `web_fetch` (or `exec` with curl when POST is required)
