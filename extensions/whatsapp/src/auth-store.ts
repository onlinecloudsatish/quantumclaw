...
import { info, success } from "quantumclaw/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "quantumclaw/plugin-sdk/state-paths";
import type { WebChannel } from "quantumclaw/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "quantumclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("quantumclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
