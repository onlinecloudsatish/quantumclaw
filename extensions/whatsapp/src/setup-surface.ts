...
import type { ChannelSetupWizard } from "quantumclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "quantumclaw/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  ...
  patch: Partial<NonNullable<NonNullable<QuantumClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: QuantumClawConfig, dmPolicy: DmPolicy): QuantumClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: QuantumClawConfig, allowFrom?: string[]): QuantumClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: QuantumClawConfig, selfChatMode: boolean): QuantumClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: QuantumClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so QuantumClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for QuantumClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("quantumclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
