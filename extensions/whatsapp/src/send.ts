...
import { generateSecureUuid } from "quantumclaw/plugin-sdk/infra-runtime";
import { normalizePollInput, type PollInput } from "quantumclaw/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "quantumclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "quantumclaw/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "quantumclaw/plugin-sdk/text-runtime";
import { toWhatsappJid } from "quantumclaw/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: QuantumClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
