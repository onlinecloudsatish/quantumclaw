...
import { resolvePreferredQuantumClawTmpDir } from "../../../src/infra/tmp-quantumclaw-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredQuantumClawTmpDir(), "quantumclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub QUANTUMCLAW_STATE_DIR.
    // Also keep it outside the QuantumClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["QUANTUMCLAW_STATE_DIR"]);
    process.env.QUANTUMCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "quantumclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredQuantumClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default QuantumClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default QuantumClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
