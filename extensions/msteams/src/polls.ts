...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["quantumclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["quantumclaw", "pollId"]) ??
    readNestedString(value, ["quantumclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "quantumclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "quantumclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          quantumclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "quantumclaw poll vote",
          displayText: "Vote recorded",
          value: { quantumclawPollId: pollId, pollId },
        },
      },
    ],
  };
