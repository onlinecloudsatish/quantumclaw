import { DEFAULT_ACCOUNT_ID } from "quantumclaw/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "quantumclaw/plugin-sdk/config-runtime";
import type { QuantumClawConfig } from "quantumclaw/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "quantumclaw/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "quantumclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "quantumclaw/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: QuantumClawConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: QuantumClawConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: QuantumClawConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: QuantumClawConfig; accountId: string }) =>
    ...
    disable: (cfg: QuantumClawConfig) => setSetupChannelEnabled(cfg, channel, false),
