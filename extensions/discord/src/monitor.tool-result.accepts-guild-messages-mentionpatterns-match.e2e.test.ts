  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/quantumclaw",
      },
    },
    session: { store: "/tmp/quantumclaw-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bquantumclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "quantumclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "QuantumClaw" },
          },
        },
        eventPatch: {
