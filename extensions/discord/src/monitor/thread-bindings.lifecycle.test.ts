...
const acpRuntime = await import("quantumclaw/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    vi.useFakeTimers();
    const previousStateDir = process.env.QUANTUMCLAW_STATE_DIR;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantumclaw-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.QUANTUMCLAW_STATE_DIR = previousStateDir;
      }
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:quantumclaw-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
