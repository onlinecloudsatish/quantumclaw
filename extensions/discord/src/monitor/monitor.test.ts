...
import { buildPluginBindingApprovalCustomId } from "quantumclaw/plugin-sdk/conversation-runtime";
import { buildAgentSessionKey } from "quantumclaw/plugin-sdk/routing";
...
vi.mock("quantumclaw/plugin-sdk/security-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/security-runtime")>();
  return {
    ...actual,
...
vi.mock("quantumclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.mock("quantumclaw/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("quantumclaw/plugin-sdk/plugin-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("quantumclaw/plugin-sdk/plugin-runtime")>();
  return {
    ...actual,
...
describe("agent components", () => {
  const createCfg = (): QuantumClawConfig => ({}) as QuantumClawConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`quantumclaw pairing approve discord ${code}`);
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): QuantumClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/quantumclaw-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "QuantumClaw App Server",
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/quantumclaw",
    });
  });
});
