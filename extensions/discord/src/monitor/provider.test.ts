...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): QuantumClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("quantumclaw/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantumclaw/plugin-sdk/plugin-runtime")>(
    "quantumclaw/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
