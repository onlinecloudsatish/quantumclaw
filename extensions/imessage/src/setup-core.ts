...
export const imessageDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: QuantumClawConfig) => cfg.channels?.imessage?.dmPolicy ?? "pairing",
  setPolicy: (cfg: QuantumClawConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
function resolveIMessageCliPath(params: { cfg: QuantumClawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    ...
    "Ensure QuantumClaw has Full Disk Access to Messages DB.",
