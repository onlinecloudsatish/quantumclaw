...
export const signalDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: QuantumClawConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "QuantumClaw"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("quantumclaw gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
