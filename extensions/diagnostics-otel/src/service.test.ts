...
import type { QuantumClawPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/quantumclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): QuantumClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("quantumclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("quantumclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("quantumclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("quantumclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("quantumclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("quantumclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("quantumclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("quantumclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("quantumclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("quantumclaw.webhook.processed");
    expect(spanNames).toContain("quantumclaw.message.processed");
    expect(spanNames).toContain("quantumclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["quantumclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("quantumclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "quantumclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["quantumclaw.reason"]).toBe("string");
    expect(String(attrs?.["quantumclaw.reason"])).not.toContain(
