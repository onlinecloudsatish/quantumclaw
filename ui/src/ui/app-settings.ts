...
import type { QuantumClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as QuantumClawApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as QuantumClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as QuantumClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as QuantumClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as QuantumClawApp);
    await loadConfig(host as unknown as QuantumClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as QuantumClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as QuantumClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as QuantumClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as QuantumClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as QuantumClawApp);
    await loadDevices(host as unknown as QuantumClawApp);
    await loadConfig(host as unknown as QuantumClawApp);
    await loadExecApprovals(host as unknown as QuantumClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as QuantumClawApp);
    await loadConfig(host as unknown as QuantumClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as QuantumClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as QuantumClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__QUANTUMCLAW_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as QuantumClawApp;
...
async function loadOverviewLogs(host: QuantumClawApp) {
...
function buildAttentionItems(host: QuantumClawApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.quantumclaw.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as QuantumClawApp, true),
    loadConfigSchema(host as unknown as QuantumClawApp),
    loadConfig(host as unknown as QuantumClawApp),
  ]);
}
