...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/quantumclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/quantumclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/quantumclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__QUANTUMCLAW_CONTROL_UI_BASE_PATH__ = "/quantumclaw";
    const app = mountApp("/quantumclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/quantumclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/quantumclaw/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("quantumclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "quantumclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/quantumclaw" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("quantumclaw.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/quantumclaw",
    });
    expect(JSON.parse(localStorage.getItem("quantumclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/quantumclaw";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/quantumclaw");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/quantumclaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/quantumclaw");
    expect(app.settings.token).toBe("");
