...
export async function handleWhatsAppStart(host: QuantumClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: QuantumClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: QuantumClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: QuantumClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: QuantumClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: QuantumClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: QuantumClawApp): string | null {
...
function buildGatewayHttpHeaders(host: QuantumClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: QuantumClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: QuantumClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: QuantumClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: QuantumClawApp) {
  const state = host.nostrProfileFormState;
