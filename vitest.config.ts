...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `quantumclaw/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "quantumclaw/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `quantumclaw/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "quantumclaw/plugin-sdk",
